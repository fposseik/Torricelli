...
class Torricelli(QMainWindow):
    ...
    def calculate_structure_factors(self):
        ...
        # the minimum and the maximum values of this energy interval could be reduced in order to speed up the calculation, provided that itÂ´s always bigger than the experimental energy range
        ...
        for i in range(int((x_max - x_min) / dx) + 1):
            ...
            ## Sample crystal:
            EH_over_EO_plus_cr = -1 * (P / np.abs(P)) * (
                eta_cr + np.sqrt(eta_cr**2 - 1)) * np.sqrt(
                    np.absolute(self.b_cr) * F_H / F_Hbar)
            EH_over_EO_minus_cr = -1 * (P / np.abs(P)) * (
                eta_cr - np.sqrt(eta_cr**2 - 1)) * np.sqrt(
                    np.absolute(self.b_cr) * F_H / F_Hbar)
            phi_plus_cr = np.arctan(
                np.imag(EH_over_EO_plus_cr) / np.real(EH_over_EO_plus_cr))
            phi_minus_cr = np.arctan(
                np.imag(EH_over_EO_minus_cr) / np.real(EH_over_EO_minus_cr))
            ...
            if refl_plus_cr > 0 and refl_plus_cr < 1:  # choose the branch that has a physically meaningful reflectivity.
                ...
                if np.real(
                        EH_over_EO_minus_cr
                ) < 0 and self.ui.radioButton_sigma_pol_light.isChecked(
                ):  # from Zegenhagen 1993 Eq. 2.10, 2.11
                ...
                # Taking change of sign in real(EH/E0) into account due to the change of sign in the polarization factor when using pi-polarization
                        ...
                        EH_over_EO_minus_cr
                ) > 0 and self.ui.radioButton_pi_pol_light.isChecked():
                    self.Theory_Phase_Sample[i] += np.pi
        ...
        if len(self.Theory_Refl_sample) == len(
            ...
            self.Pyqt_View_idealRefl.plot(
                ...
                self.Theory_Squared_Refl_Monochromator_norm,
                pen=(0, 0, 255),
                name='Refl Mono^2 norm')
            self.Pyqt_View_idealRefl.plot(
                self.Theory_photonEnergy,
                self.Theory_ReflSample_cc_ReflMono2,
                pen=(255, 0, 0),
                name='Refl Sample Cross-correl Mono^2')
        ...
        if '' != file_path:
            try:
                ...
                with open(file_path, 'r') as csvfile:
                    ...
                    csvOutdated = StrictVersion(version) < StrictVersion(
                        '3.3.273'
