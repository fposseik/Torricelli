...
#    Copyright (c) 2026 Sergey Subach
...
class QDialog_manualVal(QDialog):
...
## QDialog requesting the user to choose a group name suffix for regrouping
# used of for the Argand tab
class QDialog_removeAll(QDialog):
...
class Torricelli(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        )  # Basically reads in the whole crystal structure database once and for all
    ...
    def eventFilter(self, source, event):
        ...
        if event.type(
        ) == QtCore.QEvent.MouseButtonRelease and source is self.ui.treeWidget_Argand_List.viewport(
        ):
            self.TreeWidgetMouseButtonReleasedEvent = event.button()

        elif event.type(
        ) == QtCore.QEvent.MouseButtonDblClick and source is self.ui.treeWidget_Argand_List.viewport(
        ):
        ...
        if event.type() == QtCore.QEvent.DragEnter \
           ...
           and source is self.ui.treeWidget_Argand_List.viewport():
        ...
        ) == QtCore.QEvent.Drop and source is self.ui.treeWidget_Argand_List.viewport(
        ):
            # first proceed with the original event handling, then adapt filter
            self.ui.treeWidget_Argand_List.dropEvent(event)
            itemThatReceivedIt = self.ui.treeWidget_Argand_List.itemAt(
                event.pos())
            ...
            self.groupsChangedViaDrag.add(
                parentThatReceivedIt
            )  # group that received the item changed too
    ...
    def fwhm(self, x, y, k=10):
        ...
        if len(roots) > 2:
            raise MultiplePeaks(
                "The dataset appears to have multiple peaks, and "
    ...
    def Find_refl_and_ey_file_forAGivenFolder(self):
        ...
        for files in glob.glob(
                str(self.ui.LineEdit_CurrentWorkingDirectory.text()) + os.sep +
                '..' + os.sep + '*.refl'):
        ...
        if 1 == len(list_of_refl_files):
            self.ui.refl_name.setText(list_of_refl_files[0])
                ...
                list_of_refl_files
        ):  # Nothing found, then check folder "anything/fileNumber_comment/region_n"
            list_of_refl_files = []
            for files in glob.glob(
    ...
    def choose_dataFolder(self, folder=''):
        if '' == folder or False == folder:
            dname = QtCore.QDir.toNativeSeparators(
                QFileDialog.getExistingDirectory(
                    self, 'Select current working directory'))
        else:
            dname = folder
        ...
        if '' != dname:
            ...
            if not os.path.exists(
                    os.path.join(
                        ...
                        'results')) and os.path.exists(dname):
                os.makedirs(
                    os.path.join(
                        self.ui.LineEdit_CurrentWorkingDirectory.text(),
                        'results'))
        ...
        folder_list = os.path.normpath(
            str(self.ui.LineEdit_CurrentWorkingDirectory.text())).split(os.sep)
        self.data_file_name = folder_list[
            len(folder_list) - 2] + os.sep + folder_list[len(folder_list) - 1]
    ...
    def Save_Data_and_PlotPicture(self,
        ...
        file_name = str(self.ui.LineEdit_CurrentWorkingDirectory.text()
                       ) + os.sep + 'results' + os.sep + file_name
        #simple checks
        ...
        for index in range(nb_arrays_to_write):
            if len(data_list[index]) != len(data_list[0]):
                self.ui.statusbar.showMessage(
                    "Save_Data_and_PlotPicture() says \'Non-homogeneous lengths of arrays!\'"
                )
                return
        try:
            ...
            QMessageBox.warning(
                self, "Warning",
                'Problem writing the data to file. \"results\" folder may be missing'
            )
    ...
    def open_refl(self):
        ...
        if '' != text:
            ...
            self.display_refl()
            self.ui.statusbar.showMessage(
                'Raw reflectivity and beam intensity data are imported!', 3000)
    ...
    def open_angles(self):
        ...
        if '' != text:
            ...
            self.load_angles()
            self.ui.statusbar.showMessage(
                'Slice to angle conversion data is imported!', 3000)
    ...
    def display_refl(self):
        try:
            with open(self.ui.refl_name.text(), 'r') as refl_file:
                ...
                self.ui.display_window.setLineWrapMode(QTextEdit.NoWrap)
                self.ui.statusbar.showMessage(
                    'Raw reflectivity and beam intensity data are displayed!',
                    3000)
        except IOError:
            self.ui.statusbar.showMessage(
                '*** Problem with the reflectivity file. ***')
    ...
    def display_angles(self):
        try:
            with open(self.ui.lineEdit_angles_path.text(), 'r') as angles_file:
                ...
                self.ui.display_window.setLineWrapMode(QTextEdit.NoWrap)
                self.ui.statusbar.showMessage(
                    'Slice to angles file is displayed!', 3000)
        except IOError:
            self.ui.statusbar.showMessage(
                '*** Problem with the slice to angles file. ***')
    ...
    def Transform_R_and_EY_userFile_to_TorricelliFriendly(
        ...
        if '' == self.ui.ey_name.text():
            QMessageBox.warning(
                self, "Warning",
                "You forgot to give the experimental Electron yield file produced by CasaXPS!"
            )
            return
        if '' == self.ui.refl_name.text():
            QMessageBox.warning(
                self, "Warning",
                "You forgot to give the experimental Reflectivity file!")
            return
        ...
        data_r = np.loadtxt(
            str(self.ui.refl_name.text()), dtype="float", skiprows=1)
        ...
        if '' == columns:
            columns = '0'
        ey_column_indices = [
            x * 2 + 1 for x in np.fromstring(columns, dtype=int, sep=' ')
        ]
        # check if the indices match the number of components stored in the yield file
        if max(ey_column_indices) >= len(ey_column_names):
            QMessageBox.warning(
                self, "Warning",
                "At least the index of one component you have given exceeds the number of components stored in your yield file.<br>Check the field <i>Components to sum</i>."
            )
            return
        self.ui.column_name_label.setText(", ".join(
            [ey_column_names[x] for x in ey_column_indices]))

        data_ey = np.loadtxt(
            str(self.ui.ey_name.text()), dtype="float", skiprows=3)
        ...
        if self.ui.checkBox_AngularModeToggle.isChecked():
            np_photon_points = data_r.shape[
                0]  # number of entries for each slice
            self.nb_slices = data_ey.shape[
                0] / np_photon_points  # calculate number of slices
        ...
        if data_r.shape[0] != data_ey.shape[0]:
            QMessageBox.warning(
                self, "Warning",
                'The reflectivity and electron yield files do not have the same length!\nMaybe you intended to use angular mode?'
            )
            return

        i0 = data_r[:, 2]  # beam intensity
        i0average = sum(data_r[:, 2]) / len(
            data_r[:, 2])  # average beam intensity
        ...
        for i in range(len(
                data_r[:, 0])):  # for each photon energy, normalized with I0
            xsw_energies = np.append(xsw_energies, data_r[i, 0])
            xsw_reflectivity_normalised = np.append(
                xsw_reflectivity_normalised, data_r[i, 1] / i0[i] * i0average)
            xsw_reflectivity_estimated_error = np.append(
                xsw_reflectivity_estimated_error,
                np.sqrt(data_r[i, 1] / i0[i] * i0average))

            if abs(data_r[i, 0] - data_ey[i, 0]
                  ) > 1e-1 and not self.ui.checkBox_AngularModeToggle.isChecked(
                  ) and not self.ui.checkBox_ignore_hvCheck.isChecked():
                QMessageBox.warning(
                    self, "Warning",
                    'The photon energies of the reflectivity and electron yield files do not match!'
                )
                return
            ...
            for comp_index in range(
                    len(self.Components_List_Used_in_EYfit
                       )):  # for each fit component desired by the user
                component_number = self.Components_List_Used_in_EYfit[
                    comp_index]
                ...
                if component_col_number + 1 >= data_ey.shape[1]:
                    QMessageBox.warning(
                        self, "Warning",
                        'The component number you asked for does not exist!')
                tmp_ey_norm += data_ey[i, component_col_number]
                tmp_ey_err_squared += data_ey[
                    i, component_col_number +
                    1]**2  # sum up the squares of the errors

            xsw_ey_normalised = np.append(xsw_ey_normalised,
                                          tmp_ey_norm / i0[i] * i0average)
            xsw_ey_error_casaXPS = np.append(
                xsw_ey_error_casaXPS,
                np.sqrt(tmp_ey_err_squared) / i0[i] * i0average)
        ...
        self.Pyqt_View_raw_refl.plot(
            self.Exp_photonEnergy,
            self.Exp_Refl_Normalised,
            pen=(0, 0, 220),
            symbol='o')
        self.Pyqt_View_raw_ey.plot(
            self.Exp_photonEnergy,
            self.Exp_EY_Normalised,
            pen=(0, 0, 220),
            symbol='o')
        ey_err = pg.ErrorBarItem(
            x=self.Exp_photonEnergy,
            y=self.Exp_EY_Normalised,
            top=self.Exp_EY_casaXPS_Error,
            bottom=self.Exp_EY_casaXPS_Error,
            beam=0.05,
            pen=(200, 200, 200))
        self.Pyqt_View_raw_ey.addItem(ey_err)

        self.reset_ey_par(
        )  # Set initials parameters for the electron yield fit
        ...
        if not self.ui.lineEdit_angles_path.text(
        ) == '' and self.ui.checkBox_AngularModeToggle.isChecked():
            if len(self.slice_to_angle) != self.nb_slices:
                QMessageBox.warning(
                    self, "INCONSISTENCY DETECTED",
                    "<b>The number of angles and slices do not match the number of slices determined by your loaded yield file!</b><br><br>Please check your angles file or your yield file and make sure that they match!"
                )
            self.ui.label_AngleOfSlice.setText(
                str(self.slice_to_angle[self.ui.spinBox_SelectedSlice.value()])
                + '&deg;<b></b>')  #<b></b> is to auto enable rich text
            self.ui.doubleSpinBox_ndp_phi.setValue(
                self.slice_to_angle[self.ui.spinBox_SelectedSlice.value()])
    ...
    def allow_DCM_parameter_modification(self):
        answer = QMessageBox.warning(self, 'Editing DCM parameters',
    ...
    def refresh_sample_b_and_theta_value(self):
        ...
        self.ui.doubleSpinBox_theta.setValue(
            90. - self.ui.doubleSpinBox_sample_deviation_NI.value())
    ...
    def refresh_sample_P_value(self):
        if self.ui.radioButton_sigma_pol_light.isChecked():
            ...
            self.ui.label_pol.setText("\u03C3" + '-polarization')
            self.ui.doubleSpinBox_EYinit_Pe.setEnabled(False)
            P_electrons = 0.0  #actually not defined
        else:
            self.ui.doubleSpinBox_sample_P_Refl.setValue(
                -1 * np.cos(
                    (2 * self.ui.doubleSpinBox_sample_deviation_NI.value()) *
                    np.pi / 180.))  # same as P= cos (2 theta.)
            self.ui.label_pol.setText("\u03C0" + '-polarization')
    ...
    def determine_crystal_system(self, element, isCompound=False):
        ...
        if abs(a - b) < 1e-5 and abs(b - c) < 1e-5 and abs(
                alpha - 90) < 1e-5 and abs(beta - 90) < 1e-5 and abs(gamma -
                                                                     90) < 1e-5:
            return 'Cubic'
        elif abs(a - b) < 1e-5 and abs(b - c) > 1e-5 and abs(
                alpha - 90) < 1e-5 and abs(beta - 90) < 1e-5 and abs(
                    gamma - 120.) < 1e-5:
            return 'Hexagonal'
        elif abs(a - b) < 1e-5 and abs(b - c) > 1e-5 and abs(
                alpha - 90) < 1e-5 and abs(beta - 90) < 1e-5 and abs(gamma -
            ...
            return 'Tetragonal'
                ...
                alpha - beta) < 1e-5 and abs(beta - gamma) < 1e-5 and abs(
                    beta - 90) > 1e-5:
            return 'Trigonal'  # or 'Rhombohedral'
        elif abs(a - b) > 1e-5 and abs(b - c) > 1e-5 and abs(
                a - c) > 1e-5 and abs(alpha - 90) < 1e-5 and abs(
                    beta - 90) < 1e-5 and abs(gamma - 90) < 1e-5:
            return 'Orthorhombic'
        elif abs(a - b) > 1e-5 and abs(b - c) > 1e-5 and abs(
                a - c) > 1e-5 and abs(alpha - gamma) < 1e-5 and abs(
                    beta - 90) > 1e-5 and abs(gamma - 90) < 1e-5:
            return 'Monoclinic'
        elif abs(a - b) > 1e-5 and abs(b - c) > 1e-5 and abs(
                a - c) > 1e-5 and abs(alpha - gamma) > 1e-5 and abs(
                    alpha - beta) > 1e-5 and abs(beta - gamma) > 1e-5:
            return 'Triclinic'
        else:
            print('--> Unknown crystal system for', element)

    ## return structure name that fits the DW Databases
    def convertSystemCell_to_Abbreviations(self, element):
        if self.cell_type[
                element] == 'faceCentered' and self.determine_crystal_system(
                    element) == 'Cubic':
            return 'FCC'
        elif self.cell_type[
                element] == 'bodyCentered' and self.determine_crystal_system(
                    element) == 'Cubic':
            return 'BCC'
        elif self.cell_type[element] == 'HCP' and self.determine_crystal_system(
                element) == 'Hexagonal':
            return 'HCP'
        elif self.cell_type[
                element] == 'diamond' and self.determine_crystal_system(
                    element) == 'Cubic':
            return 'DIAMOND'
        else:
            return 'Unknnown'
    ...
    def update_structure_DCM(self):
        ...
        self.ui.label_DCM_Structure.setText(
            self.determine_crystal_system(element))
        ...
        self.ui.label_DCM_alpha.setText(
            str(self.lattice_alpha[element] * 180 / np.pi))
        self.ui.label_DCM_beta.setText(
            str(self.lattice_beta[element] * 180 / np.pi))
        self.ui.label_DCM_gamma.setText(
            str(self.lattice_gamma[element] * 180 / np.pi))
        ...
        if self.ui.doubleSpinBox_DCM_Temperature.value() > 80:
            if str(element) in self.gao_HT_Z:
                if self.gao_HT_lattice_type[
                        element] == self.convertSystemCell_to_Abbreviations(
                            element):
                    self.ui.comboBox_DCM_DWMethod.addItem('Gao')
        else:
            if str(element) in self.gao_LT_Z:
                if self.gao_LT_lattice_type[
                        element] == self.convertSystemCell_to_Abbreviations(
                            element):
                    self.ui.comboBox_DCM_DWMethod.addItem('Gao')

        if str(element) in self.sears_Z:
            if self.sears_lattice_type[
                    element] == self.convertSystemCell_to_Abbreviations(
                        element):
                self.ui.comboBox_DCM_DWMethod.addItem('Sears')
                if self.determine_crystal_system(element) == 'Cubic':
                    self.ui.comboBox_DCM_DWMethod.addItem(
                        'Warren'
                    )  # All necessary data are in Sears' paper and additionally, it must be cubic.
        self.ui.comboBox_DCM_DWMethod.addItem('None')
        self.ui.label_DCM_i.setVisible(
            str(self.ui.label_DCM_Structure.text())[0] == 'H')
    ...
    def update_structure_Sample_Elemental(self):
        ...
        element = str(
            self.ui.comboBox_Sample_Elemental_Element.itemText(currentIndex))
        self.ui.label_Sample_Elemental_Structure.setText(
            self.determine_crystal_system(element))
        self.ui.label_Sample_Elemental_Type.setText(
            str(self.cell_type[element]))
        ...
        self.ui.label_Elemental_alpha.setText(
            str(self.lattice_alpha[element] * 180 / np.pi))
        self.ui.label_Elemental_beta.setText(
            str(self.lattice_beta[element] * 180 / np.pi))
        self.ui.label_Elemental_gamma.setText(
            str(self.lattice_gamma[element] * 180 / np.pi))
        if self.checked_values[element] == 'yes':
            self.ui.label_Elemental_CheckedValue.setText('Yes')
            self.ui.label_Elemental_CheckedValue.setStyleSheet(
                'QLabel {color:green}')
        elif self.checked_values[element] == 'no':
        ...
        if str(element) in self.sears_Z:
            if self.sears_lattice_type[
                ...
                if self.determine_crystal_system(element) == 'Cubic':
                    self.ui.comboBox_Sample_Elemental_DWMethod.addItem(
                        'Warren'
                    )  # All necessary data are in Sears' paper, but it must be cubic
        self.ui.comboBox_Sample_Elemental_DWMethod.addItem('None')
        self.ui.label_Sample_Elemental_i.setVisible(
            str(self.ui.label_Sample_Elemental_Structure.text())[0] == 'H')
    ...
    def update_structure_Sample_Compound(self, currentIndex):
        element = str(
            self.ui.comboBox_Sample_Compound_Element.itemText(currentIndex))
        self.ui.label_Sample_Compound_Structure.setText(
            self.determine_crystal_system(element, True))
        self.ui.label_Sample_Compound_Type.setText(
            str(self.compound_cell_type[element]))
        ...
        self.ui.label_Compound_alpha.setText(
            str(self.compound_lattice_alpha[element] * 180 / np.pi))
        self.ui.label_Compound_beta.setText(
            str(self.compound_lattice_beta[element] * 180 / np.pi))
        self.ui.label_Compound_gamma.setText(
            str(self.compound_lattice_gamma[element] * 180 / np.pi))
        self.ui.label_Sample_Compound_i.setVisible(
            str(self.ui.label_Sample_Compound_Structure.text())[0] == 'H')
        if self.compound_checked_values[element] == 'yes':
            self.ui.label_Compound_CheckedValue.setText('Yes')
            self.ui.label_Compound_CheckedValue.setStyleSheet(
                'QLabel {color:green}')
    ...
    def interpolate_value_from_csv_file(self,
                                        fname,
                                        name_x,
                                        value_x,
                                        name_y,
    ...
    def calculate_structure_factors(self):
        ...
        if self.ui.radioButton_Sample_Elemental.isChecked(
        ):  # Elemental sample :
        ...
        b_mo = self.ui.doubleSpinBox_b_mo.value(
        )  # Usual double monochromators have two Si(111) crystals and use the (111) reflection: symmetric case, ignoring the miscut.
        self.b_cr = self.ui.doubleSpinBox_b_cr.value(
        )  # useful when the user measures off-normal reflections
        ...
        try:  # Store data in file
            with open(
                    str(self.ui.LineEdit_CurrentWorkingDirectory.text()) +
                    os.sep + "results" + os.sep + "Structure Factor.dat",
                    'w') as SFfile:
                SFfile.write('--- ' + SampleName + ' Sample parameters ---\n')
                SFfile.write('F0\t=\t' + str(F_0) + '\n')
                SFfile.write('FH\t=\t' + str(F_H) + '\n')
                SFfile.write('FHbar\t=\t' + str(F_Hbar) + '\n')
                SFfile.write('DW for element A\t=\t' + str(DW_A) +
                             ' (for elemental and compounds) \n')
                SFfile.write('DW for element B\t=\t' + str(DW_B) +
                             ' (for compounds only)\n')
        ...
        self.Theory_photonEnergy = np.arange(
            x_min, x_max + dx,
            dx)  # array of energies relative to the theoretical Bragg energy
        ...
        self.Theory_Phase_Monochromator = np.zeros(
            len(self.Theory_photonEnergy))
        
        #########################################################
        #########################################################
        #########################################################
        #########################################################
        ##            2026.04.  S.Subach    Here I introduce several variables to test formalism and equations
        ##             lines 1108 - 1160
        ...
        sqrt_eta_keyForm_real = []
        sqrt_eta_keyForm_imag = []
        ...
        for i in range(int((x_max - x_min) / dx) + 1):
            ...
            # Calculate Eta of monochromator
            ...
            ## Sample crystal:
            ######  Next few lines are from the original version, calculating ideal reflectivity and phase of the crystal // 2026.04 S.Subach
            ...
            energy.append(self.Theory_photonEnergy[i])

            eta_cr_img.append(np.imag(eta_cr))
            eta_cr_real.append(np.real(eta_cr))

            eta_cr_sqr_img.append(np.imag(eta_cr**2 - 1))
            eta_cr_sqr_real.append(np.real(eta_cr**2 - 1))

            # square root of (eta_cr**2 - 1) using key formulae https://www.cuemath.com/algebra/square-root-of-complex-number/
            abs_z = np.abs( np.sqrt(np.real(eta_cr**2 - 1)**2 + np.imag(eta_cr**2 - 1)**2) )     
            sqrt_eta_key_real = np.sqrt((abs_z + np.real(eta_cr**2 - 1))/2)
            sqrt_eta_key_imag = np.sqrt((abs_z - np.real(eta_cr**2 - 1))/2) * np.imag(eta_cr**2 - 1) / np.abs(np.imag(eta_cr**2 - 1))
            sqrt_eta_keyForm_real.append(sqrt_eta_key_real)
            sqrt_eta_keyForm_imag.append(sqrt_eta_key_imag)

            # square root of (eta_cr**2 - 1) using numpy 
            sqrt_eta_cr_img.append(np.imag(np.sqrt(eta_cr**2 - 1)))
            sqrt_eta_cr_real.append(np.real(np.sqrt(eta_cr**2 - 1)))



            E_plus_cr_img.append(np.imag(EH_over_EO_plus_cr))
            E_plus_cr_real.append(np.real(EH_over_EO_plus_cr))
            E_minus_cr_img.append(np.imag(EH_over_EO_minus_cr))
            E_minus_cr_real.append(np.real(EH_over_EO_minus_cr))

            Phase_minus_cr.append(phi_minus_cr)
            Phase_plus_cr.append(phi_plus_cr)
            Ref_minus_cr.append(refl_minus_cr)
            Ref_plus_cr.append(refl_plus_cr)

            Diff_plus.append(0)
            Diff_minus.append(0)
            if (i > 0):
                Diff_plus[i] = Ref_plus_cr[i] - Ref_plus_cr[i-1]
                Diff_minus[i] = Ref_minus_cr[i] - Ref_minus_cr[i-1]
                #print (i, '    ', Diff_plus[i], '    ', (Ref_plus_cr[i] - Ref_plus_cr[i-1]))
                Diff_plus[0] = Diff_plus[1]
                Diff_minus[0] = Diff_minus[1]
                      

            Term_10.append(F_0 / (np.sqrt(F_H*F_Hbar) ))

            if (refl_plus_cr > 0) and (refl_plus_cr):  #chacking first choce condition
                

                Term_1.append(refl_minus_cr)
                Term_2.append((phi_minus_cr)/np.pi)
                if (np.real(EH_over_EO_minus_cr) > 0):    #chacking second choce condition
                    Term_3.append((phi_minus_cr+np.pi)/np.pi)
                else:
                    Term_3.append((phi_minus_cr)/np.pi)
            else:
                Term_1.append(refl_plus_cr)
                Term_2.append(phi_plus_cr/np.pi)
                Term_3.append(phi_plus_cr/np.pi)
            ...
            ######.  Outdated (wrong) way to choose reflectivity and phase of the crystal // 2026.04 S.Subach
            ...
            ######.  Outdated (wrong) way to choose reflectivity and phase of the monochromator // 2026.04 S.Subach
            ######.  But since for monochromator in a general case it is irrelevant, I dont change this part // 2026.04 S.Subach

            ## monochromator:
            EH_over_EO_plus_mo = -1 * (P_DCM / np.absolute(P_DCM)) * (
                eta_mo + np.sqrt(eta_mo**2 - 1)) * np.sqrt(
                    np.absolute(b_mo) * F_H_DCM / F_Hbar_DCM)
            EH_over_EO_minus_mo = -1 * (P_DCM / np.absolute(P_DCM)) * (
                eta_mo - np.sqrt(eta_mo**2 - 1)) * np.sqrt(
                    np.absolute(b_mo) * F_H_DCM / F_Hbar_DCM)
            phi_plus_mo = np.arctan(
                np.imag(EH_over_EO_plus_mo) / np.real(EH_over_EO_plus_mo))
            phi_minus_mo = np.arctan(
                np.imag(EH_over_EO_minus_mo) / np.real(EH_over_EO_minus_mo))
        ...
        if True:  ######.  This is a corrected choice of reflectivity and phase // 2026.04 S.Subach

            print ('!!!!!!!!!!!!!!!!!!!!!')

            print('critical point at the index' , critical_point_index)
            print (energy[critical_point_index], '  ', sqrt_eta_cr_img[critical_point_index])
            print (energy[critical_point_index+1], '  ',sqrt_eta_cr_img[critical_point_index+1])


            Refl_Plus_Cr_at_Left_Side = 1
            for i in range(critical_point_index+1):
                if (Ref_plus_cr[i] < 0) or (Ref_plus_cr[i] > 1):
                    Refl_Plus_Cr_at_Left_Side = 0      
            if (Refl_Plus_Cr_at_Left_Side == 1):
                print('On left side Refl_Plus_Cr,  on right side Refl_Minus_Cr')
            else:
                print('On left side Refl_Minus_Cr,  on right side Refl_Plus_Cr')

            Refl_Plus_Cr_at_Right_Side = 1
            for i in range(critical_point_index+1, int((x_max - x_min) / dx) + 1):
                if (Ref_plus_cr[i] < 0) or (Ref_plus_cr[i] > 1):
                    Refl_Plus_Cr_at_Right_Side = 0
            if (Refl_Plus_Cr_at_Right_Side ==1):
                print('On left side Refl_Minus_Cr,  on right side Refl_Plus_Cr')
            else:
                print('On left side Refl_Plus_Cr,  on right side Refl_Minus_Cr')

            if (Refl_Plus_Cr_at_Right_Side == Refl_Plus_Cr_at_Left_Side):
                print('PR)BLEM')
            ...
            for i in range(critical_point_index+1):
                if (Refl_Plus_Cr_at_Left_Side == 1):
                    Refl_Theo_final.append(Ref_plus_cr[i])
                    Phase_Theo_final.append(Phase_plus_cr[i])
                    E_real_combined.append(E_plus_cr_real[i])
                else:
                    Refl_Theo_final.append(Ref_minus_cr[i])
                    Phase_Theo_final.append(Phase_minus_cr[i])
                    E_real_combined.append(E_minus_cr_real[i])
                if  (E_real_combined[i] > 0) and self.ui.radioButton_pi_pol_light.isChecked():  ###### only for pi-piolarization
                    Phase_Theo_final[i] += np.pi 
                if (E_real_combined[i] < 0) and self.ui.radioButton_sigma_pol_light.isChecked():   ###### only for sigma-piolarization
                    Phase_Theo_final[i] += np.pi 
        ...
        if False: ######.  Plotting different parameters in a separate window for debugging // 2026.04 S.Subach
            
            #print (Ref_plus_cr[0])
            print ('!!!!!!!!!!!!!!!!!!!!!')
            #print (energy[0])
            #print (np.gradient(Ref_plus_cr, energy))

            #plt.plot(energy, E_plus_cr_img, color = "green",  label = "EH_over_EO_plus_cr - Imaginary")
            #plt.plot(energy, E_plus_cr_real, color = "blue",  label = "EH_over_EO_plus_cr - Real")
            #plt.plot(energy, E_minus_cr_img, color = "green",  label = "EH_over_EO_minus_cr - Imaginary")
            #plt.plot(energy, E_minus_cr_real, color = "blue",  label = "EH_over_EO_minus_cr - Real")

            #plt.plot(energy, Phase_plus_cr, color = "black",  label = "phi_plus_cr")
            #plt.plot(energy, Phase_minus_cr, color = "orange",  label = "phi_minus_cr")
            #plt.plot(energy, Ref_plus_cr, color = "black",  label = "refl_plus_cr")
            #plt.plot(energy, Ref_minus_cr, color = "orange",  label = "refl_minus_cr")

            plt.plot(energy, Refl_Theo_final, color = "blue",  label = "Refl_Theo_combined")
            plt.plot(energy, Phase_Theo_final, color = "green",  label = "Phase_Theo_combined")
            #plt.plot(energy, E_real_combined, color = "green",  label = "EH_over_EO_minus/plus_cr")

            #plt.plot(energy, (np.gradient(Ref_plus_cr, energy)), color = "red",  label = "deriv of refl_plus_cr")
            #plt.plot(energy, (np.gradient(Ref_minus_cr, energy)), color = "blue",  label = "deriv of refl_minus_cr")

            #plt.plot(energy, Diff_plus, color = "red",  label = "diff of refl_plus_cr")
            #plt.plot(energy, Diff_minus, color = "blue",  label = "diff of refl_minus_cr")
            
            #plt.plot(energy, Term_1, color = "black",  label = "refl_combined")
            #plt.plot(energy, Term_3, color = "orange",  label = "phase_combined")

            #plt.plot(energy, eta_cr_img, color = "black",  linestyle = "dashed", label = "eta_cr_imag")
            #plt.plot(energy, eta_cr_real, color = "orange",  linestyle = "dashed", label = "eta_cr_real")

            #plt.plot(energy, eta_cr_sqr_img, color = "black",  label = "imag(eta_cr**2 - 1)")
            #plt.plot(energy, eta_cr_sqr_real, color = "orange",  label = "real(eta_cr**2 - 1)")

            #plt.plot(energy, sqrt_eta_cr_img, color = "black",  label = "imag sqrt(eta_cr**2 - 1)")
            #plt.plot(energy, sqrt_eta_cr_real, color = "orange",  label = "real sqrt(eta_cr**2 - 1)")

            #plt.plot(energy, sqrt_eta_keyForm_imag, color = "black",  label = "Imag sqrt(eta_cr**2 - 1)")
            #plt.plot(energy, sqrt_eta_keyForm_real, color = "orange",  label = "Real sqrt(eta_cr**2 - 1)")

            
            #plt.plot(energy, np.imag(Term_10), color = "black",  label = "imag F_0/sqrt(F_H*F_Hbar)")
            #plt.plot(energy, np.real(Term_10), color = "orange",  label = "real F_0/sqrt(F_H*F_Hbar)")
            ...
            plt.axvline(x=energy[critical_point_index], color='red', linestyle='-')

            plt.legend()
            plt.show()

        if True: ######.  This overwrites ranges of reflectivity and phase chosen according to the original
                 ######.   version (lines 1260-1290) and substitute by the correctly chosen ranges (lines 1360-1380).    // 2026.04 S.Subach
            for i in range(int((x_max - x_min) / dx) + 1):
                self.Theory_Refl_sample[i] = Refl_Theo_final[i]
                self.Theory_Phase_Sample[i] = Phase_Theo_final[i]
        ...
        self.Theory_Squared_Refl_Monochromator_norm = self.Theory_Refl_Monochromator**2 / sum(
            self.Theory_Refl_Monochromator**2)
        if len(self.Theory_Refl_sample) == len(
                self.Theory_Squared_Refl_Monochromator_norm):
            delta_photon = self.Theory_photonEnergy[
                1] - self.Theory_photonEnergy[0]
            ...
            full_x = np.linspace(
                -1 * len(self.Theory_photonEnergy) * delta_photon,
                (len(self.Theory_photonEnergy) - 1) * delta_photon,
                2 * len(self.Theory_photonEnergy) - 1)
            ...
            self.Theory_ReflSample_cc_ReflMono2 = interp_full(
            ...
            #self.Pyqt_View_idealRefl.addLegend(offset=(-30, -100))
            line_width_WEW = 3
            
            ###############. The following part is modified for tests.   // s.subach 2026.04
            if True:     ######. Original script 
                self.Pyqt_View_idealRefl.plot(
                    self.Theory_photonEnergy,
                    self.Theory_Phase_Sample / np.pi,
                    #pen=(0, 0, 0),
                    pen={'color':(0, 0, 0), 'width':line_width_WEW},
                    name='Phase of Sample normalized by Pi')
                self.Pyqt_View_idealRefl.plot(
                    self.Theory_photonEnergy,
                    self.Theory_Refl_sample,
                    #pen=(0, 128, 0),
                    pen={'color':(0, 140, 0), 'width':line_width_WEW},
                    name='Refl of Sample')
                self.Pyqt_View_idealRefl.plot(
                    self.Theory_photonEnergy,
                    self.Theory_Refl_Monochromator,
                    #pen=(255, 144, 0),
                    pen={'color':(255, 144, 0), 'width':line_width_WEW},
                    name='Refl of Monochromator')
                self.Pyqt_View_idealRefl.plot(
                    self.Theory_photonEnergy,
                    self.Theory_Squared_Refl_Monochromator_norm,
                    #pen=(0, 0, 255),
                    pen={'color':(0, 0, 255), 'width':line_width_WEW},
                    name='Squared Refl of Monochromator')
                self.Pyqt_View_idealRefl.plot(
                    self.Theory_photonEnergy,
                    self.Theory_ReflSample_cc_ReflMono2,
                    pen={'color':(255, 0, 0), 'width':line_width_WEW},
                    #pen=(255, 0, 0),
                    name='Refl of Sample Cross-correlated with Squared Refl of Monochromator')
   
                
               

            if False:     ########.  tests. 2026.04.  Testing values of Eta terms and critical points
                self.Pyqt_View_idealRefl.setXRange(-0.1, 0.35)
                self.Pyqt_View_idealRefl.setYRange(-1.5, 1.5)
                ...
                self.Pyqt_View_idealRefl.plot(
                    ...
                    #pen=(255, 144, 0),
                    pen={'color':(255, 144, 0), 'width':line_width_WEW},
                    name='(eta_cr**2 - 1) / imaginary part')
                self.Pyqt_View_idealRefl.plot(
                    self.Theory_photonEnergy,
                    eta_cr_sqr_real,
                    #pen=(0, 0, 255),
                    pen={'color':(0, 0, 255), 'width':line_width_WEW},
                    name='(eta_cr**2 - 1) / real part')
                self.Pyqt_View_idealRefl.plot(
                    self.Theory_photonEnergy,
                    sqrt_eta_cr_real,
                    #pen=(255, 0, 0))
                    pen={'color':(0, 125, 125), 'width':line_width_WEW},   
                    name='sqrt(eta_cr**2 - 1) / real part')
                self.Pyqt_View_idealRefl.plot(
                    self.Theory_photonEnergy,
                    sqrt_eta_cr_img,
                    pen={'color':(255, 0, 0), 'width':line_width_WEW},
                    #pen=(255, 0, 0),
                    name='sqrt(eta_cr**2 - 1) / imaginary part')
            ...
            self.ui.statusbar.showMessage(
                'Ideal Reflectivity and Phase have been successfully calculated!',
                5000)
        else:
            print(
                'You cannot make a correlation/convolution in such conditions!!!',
                len(self.Theory_Refl_sample),
                len(self.Theory_Squared_Refl_Monochromator_norm))
            return
    ...
    def calculate_structure_factor(self,
        ...
        V = 2 * a * b * c * np.sqrt(
            np.sin((alpha + beta + gamma) / 2) * np.sin(
                (-alpha + beta + gamma) / 2) * np.sin(
                    (alpha - beta + gamma) / 2) * np.sin(
                        (alpha + beta - gamma) / 2))
        ...
        cosAlphastar = (np.cos(beta) * np.cos(gamma) - np.cos(alpha)) / (
            np.sin(beta) * np.sin(gamma))
        cosBetastar = (np.cos(alpha) * np.cos(gamma) - np.cos(beta)) / (
            np.sin(alpha) * np.sin(gamma))
        cosGammastar = (np.cos(alpha) * np.cos(beta) - np.cos(gamma)) / (
            np.sin(alpha) * np.sin(beta))
        # and Eq. (1.1.2.2) and (1.1.2.3)
        d_hkl = 1 / np.sqrt((h * astar)**2 + (k * bstar)**2 + (l * cstar)**2 +
        ...
        if isMono:
            if E_bragg == 0.0:
                print(
                    'calculate_structure_factor wants a reasonable Bragg value!'
                )
        ...
        if DW_method == 'None':
            ...
            if Crystal_temp > 80:  # Eq. 2, in Ang**2
                ...
                DW_A = DW_B = B / (
                    4 * d_hkl**2
                )  # OK for Si. See Flensburg et al. PRB 60 (1999) 284 (B=0.4691 at 293K)
            else:  #T <80 Key
            ...
            if reduced_temp < 0.2:  # Eq. 14
                J = self.sears_fMin1[str(Element_A)] + (
                    (np.pi**2) / 3) * self.sears_alpha[str(Element_A)] * (
                        reduced_temp**2)
            else:
                J = 2 * self.sears_fMin2[Element_A] * reduced_temp + 1 / (
                    6 * reduced_temp) - self.sears_f2[Element_A] / (
                        360 * (reduced_temp**3))
            B = 39.904 * J / (self.sears_M[Element_A] * self.sears_vm[Element_A]
                             )  # Eq. 15, in Ang**2
            DW_A = DW_B = B / (4 * d_hkl**2)
            ...
            B_A = 8 * (np.pi)**2 * self.interpolate_value_from_csv_file(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'DW' + os.sep + 'DW_Zywietz_Si.csv', 'T',
                Crystal_temp, '<u^2_Si>') * 1e-2  # in Ang**2
            B_B = 8 * (np.pi)**2 * self.interpolate_value_from_csv_file(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'DW' + os.sep + 'DW_Zywietz_C.csv', 'T',
                Crystal_temp, '<u^2_C>') * 1e-2
            DW_A = B_A / (4 * d_hkl**2)
            DW_B = B_B / (4 * d_hkl**2)
        else:
            QMessageBox.warning(
                self, "Warning", 'Unknown Debye-Waller method: ' + DW_method +
                '.\nProceeds without Debye-Waller correction.')

        # The structure factors:
        f0_A = self.interpolate_value_from_csv_file(
            Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
            'Databases' + os.sep + 'f0.csv', '1/2dhkl', 1 / (2 * d_hkl),
            Element_A)
        f1_A = self.interpolate_value_from_csv_file(
            Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
            'Databases' + os.sep + 'f1 and f2' + os.sep +
            str(Element_A).lower() + '.nff', 'E(eV)', E_bragg, 'f1', '\t')
        f2_A = self.interpolate_value_from_csv_file(
            Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
            'Databases' + os.sep + 'f1 and f2' + os.sep +
            str(Element_A).lower() + '.nff', 'E(eV)', E_bragg, 'f2', '\t')
        if Element_B is not None:
            f0_B = self.interpolate_value_from_csv_file(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'f0.csv', '1/2dhkl', 1 / (2 * d_hkl),
                Element_B)
            f1_B = self.interpolate_value_from_csv_file(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'f1 and f2' + os.sep +
                str(Element_B).lower() + '.nff', 'E(eV)', E_bragg, 'f1', '\t')
            f2_B = self.interpolate_value_from_csv_file(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'f1 and f2' + os.sep +
                str(Element_B).lower() + '.nff', 'E(eV)', E_bragg, 'f2', '\t')
        ...
        with open(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'Lattices' + os.sep +
                'AtomCoordinates_' + Cell_type + '.csv', 'r') as atomPos:
            ...
            for row in spamreader:  # sum on all atomic positions
                ...
                if str(row['Element']) == 'A':
                    structure_factor_0 = structure_factor_0 + (
                        f1_A + 1j * f2_A
                    )  # for the (000), the corresponding d_khl is infinity, and the DW is then 0.
            ...
            if abs(structure_factor_H) < 1e-10:
                QMessageBox.warning(
                    self, "Warning", 'The ' + str(h) + str(k) + str(l) +
                    ' reflection of ' + Crystal + ' is forbidden!')
                return None, None, None, None, None, None, None, None, None
            else:
                return d_hkl, E_bragg, structure_factor_0, structure_factor_H, structure_factor_Hbar, DW_A * (
                    4 * d_hkl**2), DW_B * (4 * d_hkl**2), V, Lambda_hkl
    ...
    def loadAll_csv_Files(self):
        ...
        self.Z_inverted = {v: k for k, v in self.Z.items()
                          }  #call: self.Z_inverted[77]

        # load the lattice data for elemental samples
        with open(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'Lattices' + os.sep +
                'CrystallographicData_Elemental.csv', 'r') as lattices:
            ...
            for row in spamreader:
                ...
                self.lattice_alpha[row['Name']] = float(
                    row['alpha']) * np.pi / 180
                self.lattice_beta[row['Name']] = float(
                    row['beta']) * np.pi / 180
                self.lattice_gamma[row['Name']] = float(
                    row['gamma']) * np.pi / 180
        ...
        with open(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'Lattices' + os.sep +
                'CrystallographicData_Compound.csv', 'r') as lattices_comp:
            spamreader = csv.DictReader(
                lattices_comp, delimiter=',', quotechar='\'')
            ...
            for row in spamreader:
                ...
                self.compound_elementA[row['Name']] = self.Z_inverted[int(
                    row['Z_A'])]
                self.compound_elementB[row['Name']] = self.Z_inverted[int(
                    row['Z_B'])]
                ...
                self.compound_lattice_alpha[row['Name']] = float(
                    row['alpha']) * np.pi / 180
                self.compound_lattice_beta[row['Name']] = float(
                    row['beta']) * np.pi / 180
                self.compound_lattice_gamma[row['Name']] = float(
                    row['gamma']) * np.pi / 180
                self.compound_checked_values[row['Name']] = str(
                    row['checked_values'])

        # load the Debye-Waller according to Gao
        with open(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'DW' + os.sep +
                'DW_Gao_Elemental_highT.csv', 'r') as gao:
        ...
        with open(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'DW' + os.sep +
                'DW_Gao_Elemental_lowT.csv', 'r') as gao:
            spamreader = csv.DictReader(gao, delimiter=',')
            self.gao_LT_Z = {}
        ...
        with open(
                Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                'Databases' + os.sep + 'DW' + os.sep + 'DW_Sears.csv',
                'r') as sears:
    ...
    def Sample_ElementalCompound_choice_changed(self):
        ...
        self.ui.doubleSpinBox_Sample_Elemental_Temperature.setEnabled(
            not activate)
    ...
    def refresh_forth_Miller_Bravais_indice(self):
        self.ui.label_DCM_i.setText(
            str(-self.ui.spinBox_DCM_h.value() - self.ui.spinBox_DCM_k.value()))
        self.ui.label_Sample_Elemental_i.setText(
            str(-self.ui.spinBox_Sample_Elemental_h.value() -
                self.ui.spinBox_Sample_Elemental_k.value()))
        self.ui.label_Sample_Compound_i.setText(
            str(-self.ui.spinBox_Sample_Compound_h.value() -
                self.ui.spinBox_Sample_Compound_k.value()))
    ...
    def set_refl_par(self):
        ...
        self.ui.doubleSpinBox_ReflFit_InitVal_Norm.setValue(
            np.amax(self.Exp_Refl_Normalised) /
            np.max(self.Theory_ReflSample_cc_ReflMono2))
        self.ui.doubleSpinBox_ReflFit_InitVal_Bgd.setValue(
            self.Exp_Refl_Normalised[0])
    ...
    def write_to_refl_log_file(self, text):
        ...
        self.ui.QTextEdit_FitResult_Refl.moveCursor(QTextCursor.End)
    ...
    def residuals_Refl(self, params):
        ...
        theo = interpolate.interp1d(
            self.Theory_photonEnergy, Theory_refl, kind=interp1d_kind)
        try:
            self.Theory_refl_on_Exp_points = Norm * theo(
                self.Exp_photonEnergy_BraggCentered + DE)
        except ValueError as err:
            QMessageBox.warning(self, "ERROR", "ERROR: Failed to interpolate the experimental data."+\
        ...
        self.write_to_refl_log_file('Sigma=' + str(sigma) + '\tNorm=' +
                                    str(Norm) + '\tBgd=' + str(DR) +
                                    '\tDeltaEn=' + str(DE))
        ...
        return (
            self.Exp_Refl_Normalised - DR
        ) - self.Theory_refl_on_Exp_points  #/ np.amax(self.Exp_Refl_Normalised)

    # Fits the reflectivity
    def fit_Refl(self):
        if len(self.Exp_Refl_Normalised) == 0:
            QMessageBox.warning(self, "Warning", 'You forgot to import data !')
            return
        if len(self.Theory_photonEnergy) == 0:
            QMessageBox.warning(
                self, "Warning",
                'You forgot to compute the ideal reflectivity !')
            return
        ...
        try:
            self.log_file_name = str(
                self.ui.LineEdit_CurrentWorkingDirectory.text(
                )) + os.sep + "results" + os.sep + "Fit_refl.log"
        ...
        self.write_to_refl_log_file(
            'All the fit parameters combinations tested are reported in the following:'
        )
        self.ui.QTextEdit_FitResult_Refl.setLineWrapMode(QTextEdit.NoWrap)
        ...
        # Fit !
        bestFit_param, cov_x, info, msg, ierr = scipy.optimize.leastsq(
            self.residuals_Refl,
        ...
        if cov_x is None:
            self.write_to_refl_log_file(
                'Singular matrix encountered while optimizing')
            QMessageBox.warning(
                self, "Warning",
                'Singular matrix encountered while fitting.\n\nConsider changing the initial value of sigma and trying to fit again.'
            )
            raise NameError(
                'Fit_reflectivity: Singular matrix encountered while optimizing'
            )

        # Chi Squared according to Pearsons definition:
        ChiSq_refl = sum(
            np.divide(info['fvec']**2, self.Theory_refl_on_Exp_points)) / (
                len(self.Exp_photonEnergy_BraggCentered) - len(bestFit_param))
        ...
        fit_results_plot_note = self.data_file_name\
                                ...
                                + ' eV<br>'+"\u03C3"+'=' + str(round(bestFit_param[0]*1000., 3)) + ' meV +- (' + str(round(bestFit_dev[0]*1000., 3))\
                                ...
                                + ')<br>'+"\u03B4"+' h'+"\u03BD"+'='   + str(round(bestFit_param[3], 3)) + ' eV +- (' + str(round(bestFit_dev[3], 3))\
                                + ')<br>'+"\u03C7"+'<span style=" vertical-align:super;">2</span><span style=" vertical-align:sub;">Pearson</span>=' + str(round(ChiSq_refl, 3))
        text = pg.TextItem(
            html='<font size=\"5\">' + fit_results_plot_note + '</font>',
            color=(0, 0, 0),
            anchor=(0, 0))
        text.setPos(
            np.amin(self.Exp_photonEnergy_BraggCentered) + bestFit_param[3], 1)
        ...
        self.Pyqt_View_refl_fit.plot(
            ...
            pen=(255, 0, 0))
        self.Pyqt_View_refl_fit.plot(
            self.Theory_photonEnergy, bestFit_theo_refl, pen=(0, 0, 255))
        plot_center = self.Theory_photonEnergy[np.argmax(
            self.Theory_ReflSample_cc_ReflMono2)]
        plot_width = (self.Exp_photonEnergy_BraggCentered[-1] -
                      self.Exp_photonEnergy_BraggCentered[0]) * 1.2
        self.Pyqt_View_refl_fit.setXRange(plot_center - plot_width / 2.,
                                          plot_center + plot_width / 2.)
        self.Pyqt_View_refl_fit.setYRange(0, 1)
        ...
        self.ui.statusbar.showMessage(
            'Experimental Reflectivity has been successfully fitted!', 5000)

    #######################################################
    #########    Section Fit EY: CF and CP   ##############
    #######################################################

    def EY_initSlider_valueChanged(
            self, who):  # val not used because of actionTriggered
        if 'fc' == who:
    ...
    def set_fitEYparam_forManualUse(self):
        self.ui.doubleSpinBox_fc.setValue(
            self.ui.doubleSpinBox_EYFit_fc.value())
        self.ui.doubleSpinBox_pc.setValue(
            self.ui.doubleSpinBox_EYFit_pc.value())
        ...
        if self.ui.checkBox_eyfit_fitgamma.isChecked():
            ...
            self.ui.doubleSpinBox_EYinit_man_gamma.setValue(
                self.ui.doubleSpinBox_EYFit_gamma.value())
        if self.ui.checkBox_eyfit_fitSr.isChecked():
            self.ui.radioButton_EYinit_man_SR.setChecked(True)
            self.ui.doubleSpinBox_EYinit_man_SR.setValue(
                self.ui.doubleSpinBox_EYFit_Sr.value())
    ...
    def write_line_EY_log_file(self, line):
        ...
        self.ui.QTextEdit_FitResult_EY.moveCursor(QTextCursor.End)
    ...
    def update_EY_manual_plot(self):
        if self.ui.button_fit_ey.isEnabled(
        ) and 0 != self.ui.doubleSpinBox_EYFit_n.value():
            ...
            params.add(
                'Delta', value=self.ui.doubleSpinBox_EYinit_Delta.value())
            ...
            red_chi_sq = sum(residuals**2) / (
                len(self.Exp_EY_Normalised) - (len(params) - 1)
            )  # does not fit exactely to the value what 'minimize()' computes, but it's close enough...
            self.ui.doubleSpinBox_manual_chisq.setValue(red_chi_sq)
            if self.ui.checkBox_display_manual_EY.isChecked():
                self.the_plot_EY_manual.setData(self.Theory_photonEnergy,
                                                theoretical_EY_curve)
    ...
    def update_EY_plot(self, with_manual_spectrum=True):
        ...
        text = pg.TextItem(
            html='<font size=\"5\">' + self.fit_results_plot_note + '</font>',
            color=(0, 0, 0),
            anchor=(0, 0))
        text.setPos(
            np.amin(self.Exp_photonEnergy_BraggCentered - 0.5) +
            self.ui.doubleSpinBox_ReflFit_de.value(), 4)
        self.Pyqt_View_ey_fit.addItem(text)
        if self.ui.checkBox_display_manual_EY.isChecked(
        ) and with_manual_spectrum:
            self.the_plot_EY_manual = self.Pyqt_View_ey_fit.plot(
                [0, 1], [1, 2], pen=(255, 0, 0)
            )  #self.Exp_photonEnergy, self.Exp_EY_Normalised/fitted_N, pen=(255,0,0))
            self.update_EY_manual_plot()
        self.the_plot_EY_fit = self.Pyqt_View_ey_fit.plot(
            self.Theory_photonEnergy, self.Fit_Result_EY, pen=(0, 0, 255))
        self.the_plot_EY_exp = self.Pyqt_View_ey_fit.plot(
            self.Exp_photonEnergy_BraggCentered +
            self.ui.doubleSpinBox_ReflFit_de.value(),
            self.Exp_EY_Normalised / fitted_N,
            pen=None,
            symbol='o',
            symbolBrush=(0, 150, 255))
        ...
        self.Pyqt_View_ey_fit.setXRange(
            np.amin(self.Exp_photonEnergy_BraggCentered) - 0.5 +
            self.ui.doubleSpinBox_ReflFit_de.value(),
            np.amax(self.Exp_photonEnergy_BraggCentered) + 0.5 +
            self.ui.doubleSpinBox_ReflFit_de.value())
    ...
    def residual_ey(self, parameters, manual=False):
        ...
        Theo_Sample_EY = (
            1 + Sr * self.Theory_Refl_sample +
            2 * Fc * Si * np.sqrt(self.Theory_Refl_sample) *
            np.cos(self.Theory_Phase_Sample - 2 * np.pi * Pc + Psi))
        Theo_Sample_EY_cc_Gauss = np.correlate(Theo_Sample_EY,\
                                               ...
                                               mode = 'same') # with a centered gaussian, the axis does not change
        Theo_Sample_EY_cc_Gauss_cc_RMono2_full = np.correlate(
            Theo_Sample_EY_cc_Gauss,
            self.Theory_Squared_Refl_Monochromator_norm,
            mode='full')

        delta_photon = self.Theory_photonEnergy[1] - self.Theory_photonEnergy[0]
        full_x = np.linspace(-1 * len(self.Theory_photonEnergy) * delta_photon,
                             (len(self.Theory_photonEnergy) - 1) * delta_photon,
                             2 * len(self.Theory_photonEnergy) - 1)
        interp_full = interpolate.interp1d(
            full_x, Theo_Sample_EY_cc_Gauss_cc_RMono2_full, kind=interp1d_kind)
        self.Theo_Sample_EY_cc_Gauss_cc_RMono2 = interp_full(
            self.Exp_photonEnergy_BraggCentered +
            self.ui.doubleSpinBox_ReflFit_de.value())

        if parameters['Sr'].vary:
            self.write_line_EY_log_file('Fc=' + str(Fc) + '\tPc=' + str(Pc) +
                                        '\tN=' + str(N) + '\tSr=' + str(Sr))
        elif parameters['gamma'].vary:
            self.write_line_EY_log_file('Fc=' + str(Fc) + '\tPc=' + str(Pc) +
                                        '\tN=' + str(N) + '\tgamma=' +
        ...
        if manual:  #return both the difference and the theoretical curve
            if self.ui.checkBox_ignore_MonteCarlo.isChecked():
                return (self.Exp_EY_Normalised / N -
                        self.Theo_Sample_EY_cc_Gauss_cc_RMono2), interp_full(
                            self.Theory_photonEnergy)
            else:
                return (self.Exp_EY_Normalised / N -
                        self.Theo_Sample_EY_cc_Gauss_cc_RMono2) / (
                            self.Exp_EY_casaXPS_Error / N), interp_full(
                                self.Theory_photonEnergy)
        else:
            if self.ui.checkBox_ignore_MonteCarlo.isChecked():
                return (self.Exp_EY_Normalised / N -
    ...
    def fit_ElYield(self):
        ...
        if self.ui.checkBox_AngularModeToggle.isChecked():
            self.path_fit_ey_log = str(
                self.ui.LineEdit_CurrentWorkingDirectory.text()
            ) + os.sep + 'results' + os.sep + 'Fit_ey_comp' + str(
                self.Components_List_Used_in_EYfit
            ) + '_slice%02i' % self.ui.spinBox_SelectedSlice.value() + '.log'
        else:
            self.path_fit_ey_log = str(
                self.ui.LineEdit_CurrentWorkingDirectory.text(
                )) + os.sep + 'results' + os.sep + "Fit_ey_comp" + str(
                    self.Components_List_Used_in_EYfit) + '.log'
        ...
        fit_params.add(
            'Fc',
            value=self.ui.doubleSpinBox_fc.value(),
            vary=self.ui.checkBox_eyfit_fitFc.isChecked())
        fit_params.add(
            'Pc',
            value=self.ui.doubleSpinBox_pc.value(),
            vary=self.ui.checkBox_eyfit_fitPc.isChecked())
        fit_params.add(
            'N',
            value=self.ui.doubleSpinBox_n.value(),
            vary=self.ui.checkBox_eyfit_fitN.isChecked())
        fit_params.add(
            'Delta',
            value=self.ui.doubleSpinBox_EYinit_Delta.value(),
            vary=False)
        fit_params.add(
            'Sr',
            value=self.ui.doubleSpinBox_EYinit_man_SR.value(),
            vary=self.ui.checkBox_eyfit_fitSr.isChecked() and
            self.ui.radioButton_EYinit_man_SR.isChecked())
        fit_params.add(
            'gamma',
            value=self.get_init_value_gamma(),
            vary=self.ui.checkBox_eyfit_fitgamma.isChecked() and
            (not self.ui.radioButton_EYinit_man_SR.isChecked()))
        fit_result_output = lmfit.minimize(
            self.residual_ey, fit_params, args=())

        if fit_result_output.success is not True:
            self.write_line_EY_log_file('Fit did not converge:' +
                                        fit_result_output.message)
            QMessageBox.warning(
                self, "Fit does not converge",
                'Try to change the parameter start values.\n\"' +
                fit_result_output.message + '\"')
            return
        else:
            self.write_line_EY_log_file('Successful fit: \"' +
                                        fit_result_output.message + '\"')
            self.ui.statusbar.showMessage(
                'Successful fit: \"' + fit_result_output.message + '\"', 5000)

        # Calculation of R^2, goodness of fit
        self.R_squared_ey = self.calculate_the_R_squared_parameter(
        ...
        if self.ui.checkBox_ignore_MonteCarlo.isChecked():
            ...
            self.ui.doubleSpinBox_FitEY_RedChiSquare.setValue(
                fit_result_output.redchi)

        self.ui.doubleSpinBox_EYFit_fc.setValue(
            fit_result_output.params['Fc'].value)
        self.ui.doubleSpinBox_EYFit_pc.setValue(
            fit_result_output.params['Pc'].value)
        self.ui.doubleSpinBox_EYFit_n.setValue(
            fit_result_output.params['N'].value)
        self.ui.doubleSpinBox_stddev_ey_fc.setValue(
            fit_result_output.params['Fc'].stderr)
        self.ui.doubleSpinBox_stddev_ey_pc.setValue(
            fit_result_output.params['Pc'].stderr)
        self.ui.doubleSpinBox_stddev_n.setValue(
            fit_result_output.params['N'].stderr)
        ...
        if self.ui.checkBox_eyfit_fitgamma.isChecked():
            ...
            self.ui.doubleSpinBox_EYFit_gamma.setValue(
                fit_result_output.params['gamma'].value)
            self.ui.doubleSpinBox_stddev_gamma.setValue(
                fit_result_output.params['gamma'].stderr)
            Sr, Si, Psi, Q_0, Q_h = self.compute_ndp_from_gamma(
                fit_result_output.params['gamma'].value,
                self.ui.doubleSpinBox_EYinit_Delta.value())
            ...
            self.ui.doubleSpinBox_EYFit_Sr.setValue(
                fit_result_output.params['Sr'].value)
            self.ui.doubleSpinBox_stddev_Sr.setValue(
                fit_result_output.params['Sr'].stderr)

        self.write_line_EY_log_file("Reduced chi squared = " +
                                    str(fit_result_output.redchi))
        ...
        for var in fit_result_output.params:
            self.write_line_EY_log_file(
                '--> ' + var + ' with ' +
                str(fit_result_output.params[var].correl))
        ...
        self.fit_results_plot_note = self.data_file_name + selected_slice\
                                     ...
                                     + ')<br>'+"\u03C3"+'=' + str(round(self.ui.doubleSpinBox_ReflFit_sigma.value()*1000, 3)) + ' meV'\
                                     + '<br>'+"\u03C7"+'<span style=" vertical-align:super;">2</span><span style=" vertical-align:sub;">Red.</span>='   + str(round(self.ui.doubleSpinBox_FitEY_RedChiSquare.value(), 3))\
                                     + '<br>Comp. ' + str(self.Components_List_Used_in_EYfit)+'('+self.ui.column_name_label.text()+')'

        dummy, self.Fit_Result_EY = self.residual_ey(fit_result_output.params,
                                                     True)
        self.update_EY_plot(False)
        self.the_plot_EY_fit.setData(self.Theory_photonEnergy,
                                     self.Fit_Result_EY)

        if self.ui.checkBox_AngularModeToggle.isChecked():
            name_fit_ey = "Fit_ey_comp" + str(
                self.Components_List_Used_in_EYfit
            ) + '_slice%02i' % self.ui.spinBox_SelectedSlice.value()
            name_exp_ey = 'Exp_ey_norm_centred' + str(
                self.Components_List_Used_in_EYfit
            ) + '_slice%02i' % self.ui.spinBox_SelectedSlice.value()
        else:
            name_fit_ey = "Fit_ey_comp" + str(
        ...
        self.ui.statusbar.showMessage(
            'Experimental Electron Yield has been successfully fitted!', 5000)
    ...
    def update_component_list(self):
        self.Components_List_Used_in_EYfit = np.fromstring(
            str(self.ui.signal_name.text()), dtype=int, sep=' ')
    ...
    def autoSaveResults(self,
        ...
        if len(kwargs) == 0:
            ...
            saveFilePath = str(self.ui.LineEdit_CurrentWorkingDirectory.text()
                              ) + os.sep + 'results' + os.sep + filename
            ...
            if os.path.isfile(saveFilePath):
                try:
                    ...
                    print(
                        "There was an Error in generating the result file.\nfile %s, line %d: %s"
                        % (saveFilePath, reader.line_num, e))
            ...
            try:
                with open(saveFilePath, 'w') as saveFile:
                    ...
                    CSVwriter = csv.DictWriter(
                        saveFile,
                        fieldnames=self.Argand_ColList + ['Path'],
                        delimiter=';',
                        quoting=csv.QUOTE_NONNUMERIC)
            ...
            self.ui.statusbar.showMessage("Results of fit appended to " +
                                          filename)
    ...
    def look_for_gamma_file(self):
        ...
        if os.path.exists(gamma_iniFile):
            ...
            self.elements_in_gammaINI = ["None"
                                        ] + self.parserGamma.sections()[1:]
            self.ui.comboBox_NonDipolar_Element.addItem(
                self.elements_in_gammaINI[0])
            ...
            print(
                "ERROR: Database file containing gamma parameters file not found found!"
            )
    ...
    def display_Gamma_file(self, resetQuery):
        ...
        if resetQuery != -1 and self.currentElem != 0:
            ...
            self.E_b = float(
                ast.literal_eval(
                    self.parserGamma.get(
                        self.elements_in_gammaINI[self.currentElem],
                        "eb"))[shellIndex])
            self.E_kin = ast.literal_eval(
                self.parserGamma.get(
                    self.elements_in_gammaINI[self.currentElem], "e_kin"))
            self.gammaParams = ast.literal_eval(
                self.parserGamma.get(
                    self.elements_in_gammaINI[self.currentElem],
                    "gamma_params"))[shellIndex]
            coreLevel = str(self.ui.comboBox_NonDipolar_Element.currentText(
            )).strip() + self.ui.comboBox_NonDipolar_Shells.currentText()

            self.ui.textEdit_gamma.append("%s\t%s" %
                                          ("CoreLevel", "BindingEnergy[eV]"))
    ...
    def nonDipolar_changeShell(self):
        ...
        if self.currentElem != 0:
            ...
            self.currentShells = ast.literal_eval(
                self.parserGamma.get(
                    self.elements_in_gammaINI[self.currentElem], "shells"))
            self.ui.comboBox_NonDipolar_Shells.addItems(self.currentShells)

    def saveGammaFile(self):
        saveDirectory, _ = QFileDialog.getSaveFileName(
            self, "Save Gamma File",
            self.ui.LineEdit_CurrentWorkingDirectory.text(),
            "data files (*.dat);; All (*)")
        if saveDirectory != '':
            with open(saveDirectory, 'w') as gammaFile:
                gammaFile.write(str(self.ui.textEdit_gamma.toPlainText()))
            self.ui.statusbar.showMessage('Gamma parameters exported to file!',
                                          3000)
    ...
    def updatePhaseShiftDifference(self):
        ...
        self.ui.doubleSpinBox_EYinit_Delta.setValue(
            self.ui.doubleSpinBox_ndp_delta_d.value() -
            self.ui.doubleSpinBox_ndp_delta_p.value())

    def setPhotoelectronKineticEnergy(self):
        if self.ui.comboBox_NonDipolar_Element.currentIndex(
        ) == 0 and self.ui.radioButton_DQ_approx.isChecked():
            print(
                'You forgot to choose an element for the photoelectron kinetic energy calculation!'
            )
            self.E_b = 0
        self.ui.doubleSpinBox_ndp_Ekin.setValue(
            self.ui.doubleSpinBox_ndp_EBragg.value() - self.E_b)
    ...
    def setGammaValue(self):
        if self.ui.radioButton_D_approx.isChecked():  # Dipole approx only
            ...
            if self.ui.doubleSpinBox_ndp_Ekin.value() < 0:
                QMessageBox.warning(
                    self, "Warning",
                    'The photon energy is too small for this core-level.')
                return
            if self.ui.lineEdit_angles_path.text() == '':
                QMessageBox.warning(self, "ERROR", "It seems like that you have not loaded an angle file that connects the slices to certain angles.\n\n "\
            ...
            try:
                ...
                if given_E_kin < self.E_kin[0] or given_E_kin > self.E_kin[-1]:
                    QMessageBox.warning(
                        self, "ERROR",
                        "Given kinetic Energy is out of bounds\nExpect it to be between "
                        + str(self.E_kin[0]) + " and " + str(self.E_kin[-1]) +
                        " eV")
                    self.ui.radioButton_EYinit_man_gamma.setEnabled(True)
                    return
                else:
                ...
                QMessageBox.warning(self, "Warning", str(warn))

    ## Function separated from theo_ndp() because it is to be used by residual_ey()
    def compute_ndp_from_gamma(self, gamma, Delta):
        electron_emission_angle = self.ui.doubleSpinBox_ndp_phi.value(
        ) * np.pi / 180  # \phi
        Reflection_angle = 2 * self.ui.doubleSpinBox_sample_deviation_NI.value(
        ) * np.pi / 180  # 2 times \xi
        if self.ui.radioButton_pi_pol_light.isChecked(
        ):  #Both for dipole AND dipole+quadrupole!
            Q_0 = gamma * np.cos(electron_emission_angle) / 3.
            Q_h = gamma * np.cos(electron_emission_angle -
                                 Reflection_angle) / 3.
            P_electrons = self.ui.doubleSpinBox_EYinit_Pe.value()
            Sr = P_electrons * P_electrons * (1 + Q_h) / (1 - Q_0)
            Si, Psi = cmath.polar(P_electrons *
                                  (1 + (Q_h - Q_0) / 2. + 1j * np.tan(Delta) *
                                   (Q_h + Q_0) / 2.) / (1 - Q_0))
    ...
    def setYieldParameters(self):
        if self.ui.radioButton_EYinit_theo_gamma.isChecked():
            gamma = self.ui.doubleSpinBox_EYinit_theo_gamma.value()
        elif self.ui.radioButton_EYinit_man_gamma.isChecked():
            gamma = self.ui.doubleSpinBox_EYinit_man_gamma.value()
        else:
            gamma = 0

        Sr, Si, Psi, Q_0, Q_h = self.compute_ndp_from_gamma(
            gamma, self.ui.doubleSpinBox_EYinit_Delta.value())
    ...
    def ndp_choose_work_with_SR_or_gamma(self):
        ...
        if self.ui.checkBox_eyfit_fitFc.isChecked(
        ) or self.ui.checkBox_eyfit_fitPc.isChecked():
        ...
        if not self.ui.checkBox_eyfit_fitFc.isChecked() and abs(
                self.ui.doubleSpinBox_fc.value()) < 1e-3:  #Fc=0 and fixed
            ...
            if self.ui.radioButton_EYinit_theo_gamma.isChecked(
            ) or self.ui.radioButton_EYinit_man_gamma.isChecked():
                self.ui.checkBox_eyfit_fitgamma.setEnabled(True)
                self.ui.checkBox_eyfit_fitSr.setEnabled(False)
                self.ui.doubleSpinBox_EYinit_man_SR.setEnabled(False)
                if self.ui.checkBox_eyfit_fitgamma.isChecked():
                    self.ui.doubleSpinBox_EYinit_theo_SR_fitResult.setEnabled(
                        True)
                    self.ui.label_sr_from_gamma_fitResult.setEnabled(True)
                else:
                    self.ui.doubleSpinBox_EYinit_theo_SR_fitResult.setEnabled(
        ...
        if not self.ui.checkBox_eyfit_fitFc.isChecked(
        ) and not self.ui.checkBox_eyfit_fitPc.isChecked():  #Fc and Pc fixed
            if self.ui.radioButton_EYinit_theo_gamma.isChecked(
            ) or self.ui.radioButton_EYinit_man_gamma.isChecked():
                self.ui.checkBox_eyfit_fitgamma.setEnabled(True)
            elif self.ui.radioButton_EYinit_man_SR.isChecked():
    ...
    def Argand_AddGroup(self, GroupDict_new, refresh=False):
        ...
        if GroupDict_new is False:  # Called through the 'Add group' button
            group_name, group_confirmed = QInputDialog.getText(
                self, 'Create a new group', 'Group name:')
            if group_confirmed:
        ...
        item = HashableQTreeWidgetItem(self.ui.treeWidget_Argand_List)
        self.ui.treeWidget_Argand_List.setCurrentItem(item)
        if 'checkState' in GroupDict:  # pass checkState if known
            item.setCheckState(
                self.Argand_col['Name'], QtCore.Qt.Checked if
                GroupDict['checkState'] == 'checked' else QtCore.Qt.Unchecked)
        else:
        ...
        if type(GroupDict['Color']) == str or type(GroupDict['Color']) == tuple:
            ...
            item.setBackground(self.Argand_col['Color'],
                                    QColor(color[0], color[1], color[2]))
        elif type(GroupDict['Color']) == QColor:
            item.setBackground(self.Argand_col['Color'],
                                    GroupDict['Color'])
        else:
            print('Unkonwn group color type:', type(GroupDict['Color']))
        #item.setToolTip(   self.Argand_col['Name'], os.path.basename(str(DataDict['Path'])))
        item.setIcon(
            0,
            QIcon(Torricelli_program_folder_path + os.sep + 'imports' + os.sep +
                  'Torricelli_icon.png'))
    ...
    def Argand_AddDataset(self, gp, DataDict, refresh=False):
        if DataDict['Fc_err'] == '':
            DataDict['Fc_err'] = 0.0001
        if DataDict['Pc_err'] == '':
            DataDict['Pc_err'] = 0.0001
        self.Argand_QTreeWidgetSignalsSwitch(on=False)
        child = HashableQTreeWidgetItem(gp)
        child.setFlags(
            QtCore.Qt.ItemFlags(QtCore.Qt.ItemIsSelectable
                                | QtCore.Qt.ItemIsEnabled
        ...
        if 'checkState' in DataDict:  # pass checkState if known
            child.setCheckState(
                self.Argand_col['Name'], QtCore.Qt.Checked
                if DataDict['checkState'] == 'checked' else QtCore.Qt.Unchecked)
        else:
        ...
        if type(DataDict['Color']) == str or type(DataDict['Color']) == tuple:
            ...
            child.setBackground(self.Argand_col['Color'],
                                     QColor(color[0], color[1], color[2]))
        elif type(DataDict['Color']) == QColor:
            child.setBackground(self.Argand_col['Color'],
                                     DataDict['Color'])
        else:
            print('Unkonwn point color type:', type(DataDict['Color']))
        ...
        child.setText(self.Argand_col['Slice nb'],
                      str(self.intOrEmpty(DataDict['Slice nb'])))
        child.setText(self.Argand_col['delta hnu'], str(DataDict['delta hnu']))
        child.setText(self.Argand_col['Yield file'],
                      str(DataDict['Yield file']))
        child.setText(self.Argand_col['Note'], str(DataDict['Note']))
        child.setText(self.Argand_col['Core level'],
                      str(DataDict['Core level']))
        ...
        child.setText(self.Argand_col['Monte Carlo analysis'],
                      str(DataDict['Monte Carlo analysis']))
        ...
        if child.checkState(self.Argand_col['Name']) == QtCore.Qt.Checked:
            try:
                self.argand.addDataSet(data_argand=[float(child.text(self.Argand_col['Pc'])), float(child.text(self.Argand_col['Fc']))],\
                                       ...
                                       color=child.background(self.Argand_col['Color']).color().getRgb()[0:3],\
        ...
        if child.text(self.Argand_col['Pc']) == '-' or float(
                child.text(self.Argand_col['Fc'])) < 1e-8:
        ...
        if refresh:
            self.Argand_groupAverage([
                gp
            ])  # recalculate group vector and replot it in argand diagram
    ...
    def Argand_updateItems(self, item_list, col=None, recalcGP=True):
        ...
        for item in item_list:
            ...
            if recalcGP and self.Argand_isDataPoint(item):
                self.Argand_groupAverage(
                    gp_items=[item.parent()], refresh=False)
            ...
            if self.Argand_isGroup(item):
                ...
                for i in range(item.childCount()):
                    item.child(i).setCheckState(
                        self.Argand_col['Name'],
                        item.checkState(self.Argand_col['Name']))
                if item.checkState(
                        self.Argand_col['Name']) == QtCore.Qt.Checked:
                ...
                if item.checkState(
                    ...
                    try:
                        self.argand.addDataSet(data_argand=[float(item.text(self.Argand_col['Pc'])), float(item.text(self.Argand_col['Fc']))],\
                                               ...
                                               drawError=self.ui.checkBox_Argand_display_errorBars.isChecked(),\
                                               color=item.background(self.Argand_col['Color']).color().getRgb()[0:3],\
    ...
    def Argand_Remove(self, item_list):
        ...
        for i in range(len(item_list)):
            if i < 10:
                name = name + str(item_list[i].text(
                    self.Argand_col['Name'])) + '\n'
        if len(item_list) >= 10:
            name = name + '...'
        ...
        if reply == QMessageBox.Yes:
            ...
            for currentItem in item_list_ch:  # Iterate through all points
                if not self.Argand_isGroup(currentItem):
                    ...
                    sip.delete(
                        currentItem
                    )  # using the C++ 'delete' command... works fine and immediately
    ...
    def Argand_RemoveAll(self):
        ...
        if dialog.exec_():
            self.Argand_Save(
                str(self.ui.LineEdit_CurrentWorkingDirectory.text()) + os.sep +
                self.argand_SaveSubFolder + 'autosave_' + self.timestamp() +
                '_beforeRemoveAll.csv')
    ...
    def Argand_ValueLogFile(self):
        ...
        if current_gp is not None and self.Argand_isGroup(current_gp):
            dataset_names, _ = QFileDialog.getOpenFileNames(
                self, "Open Files",
                self.ui.LineEdit_CurrentWorkingDirectory.text(),
                ...
                if dataset_name != '':
                    try:
                        with open(dataset_name, 'r') as log_file:
                            ...
                            if 5 == len(
                                    lastLine.split('\t')
                            ):  # Output from old Torricelli version (before r70)
                                dic.update({'Fc'        : lastLine.split('\t')[0].split('=')[1],\
                                            ...
                                            'Path'      : dataset_name,\
                                            'Color'     : current_gp.background(self.Argand_col['Color']),\
                                            'Symbol'    : 'o'})
                                self.Argand_AddDataset(current_gp, dic)
                            else:
                                QMessageBox.warning(
                                    self, "Warning",
                                    'You are trying to open a log file that EITHER did not converge OR has an unknown format'
                                )
                    except IOError as e:
                        QMessageBox.warning(self, "Warning",
                                            "Problem with the selected file.")
        ...
        if current_gp is not None and self.Argand_isGroup(current_gp):
            ...
            if dialog.exec_():  # only if OK is clicked
                ...
                dic.update({'Pc'        : dialog.ui.doubleSpinBox_pc.value(),\
                            ...
                            'Symbol'    : 'o',\
                            'Color'     : current_gp.background(self.Argand_col['Color']).color().getRgb()[0:3],\
                            'Name'      : name})
                self.Argand_AddDataset(current_gp, dic)
    ...
    def Argand_replotDiagram(self):
        ...
        progressLabel0 = QLabel()
        progressLabel1 = QLabel()
        progressLabel2 = QLabel()
        progressBarGROUP = QProgressBar()
        progressBarCHILD = QProgressBar()
        ...
        for i in range(child_count):  # Iterate on the groups
            ...
            if gp.checkState(self.Argand_col['Name']) == QtCore.Qt.Checked:
                if str(gp.text(self.Argand_col['Pc_err'])) == '-':
                    g_Pcerr = 0.0
                else:
                    g_Pcerr = float(gp.text(self.Argand_col['Pc_err']))
                if str(gp.text(self.Argand_col['Fc_err'])) == '-':
                    g_Fcerr = 0.0
                else:
                    g_Fcerr = float(gp.text(self.Argand_col['Fc_err']))
                try:
                    self.argand.addOriginVector(pc=float(gp.text(self.Argand_col['Pc'])),\
                                                ...
                                                color=gp.background(self.Argand_col['Color']).color().getRgb()[0:3],\
                                                ident=id(gp))
                ...
                for j in range(gp_children):  # Iterate in all points in a group
                    ...
                    if point.checkState(
                        ...
                        if str(gp.text(self.Argand_col['Pc_err'])) == '-':
                            ...
                            Pcerr = float(point.text(self.Argand_col['Pc_err']))
                        if str(gp.text(self.Argand_col['Fc_err'])) == '-':
                            Fcerr = 0.0
                        else:
                            Fcerr = float(point.text(self.Argand_col['Fc_err']))
                        ...
                        try:
                            self.argand.addDataSet(data_argand=[float(point.text(self.Argand_col['Pc'])), float(point.text(self.Argand_col['Fc']))],\
                                                   ...
                                                   drawError=self.ui.checkBox_Argand_display_errorBars.isChecked(), \
                                                   color=point.background(self.Argand_col['Color']).color().getRgb()[0:3], \
                    ...
                    progressBarGROUP.setValue(
                        round(float(i + 1) / child_count * 100))
                    progressBarCHILD.setValue(
                        round(float(j + 1) / gp_children * 100))
                    # old but gold:
    ...
    def Argand_replotGroupAverage(self, item):
        ...
        elif self.Argand_isGroup(item) and item.childCount() == 0 and id(
                item) in self.argand.originVectorDict:
            self.argand.remove_originVector(ident=id(item))
    ...
    def Argand_groupAverage(self, gp_items=[], refresh=True):
        ...
        if len(gp_items) == 0:
            gp_items = list(map(root.child, list(range(root.childCount()))))
        if root.childCount() == 0:
            return  # There is nothing at all!
        ...
        for current_gp in gp_items:  # Iterate on the groups
            for child_index in range(current_gp.childCount()):
                if current_gp.child(child_index).text(
                        self.Argand_col['Pc_err']
                ) == '-' or current_gp.child(child_index).text(
                        self.Argand_col['Fc_err']
                ) == '-' or current_gp.child(child_index).text(
                        self.Argand_col['Pc_err']
                ) == '' or current_gp.child(child_index).text(
                        self.Argand_col['Fc_err']) == '' or float(
                            current_gp.child(child_index).text(
                                self.Argand_col['Pc_err'])) < 1e-8 or float(
                                    current_gp.child(child_index).text(
        ...
        for current_gp in gp_items_with_possible_averaging:  # Iterate on the groups
            if current_gp.childCount() == 0:
                ...
                current_gp.setText(
                    self.Argand_col['Pc'],
                    current_gp.child(0).text(self.Argand_col['Pc']))
                current_gp.setText(
                    self.Argand_col['Fc'],
                    current_gp.child(0).text(self.Argand_col['Fc']))
                current_gp.setText(
                    self.Argand_col['Pc_err'],
                    current_gp.child(0).text(self.Argand_col['Pc_err']))
                current_gp.setText(
                    self.Argand_col['Fc_err'],
                    current_gp.child(0).text(self.Argand_col['Fc_err']))
                ...
                for child_index in range(current_gp.childCount()):
                    try:
                        ...
                        QMessageBox.warning(
                            self, "ERROR",
                            "Your coherent position or coherent fraction is not computable. Check your yield fit!"
                        )
                    pc_err_child = float(
                        current_gp.child(child_index).text(
                            self.Argand_col['Pc_err']))
                    fc_err_child = float(
                        current_gp.child(child_index).text(
                            self.Argand_col['Fc_err']))
                    err_child_Re = np.sqrt(
                        (np.cos(2 * np.pi * child_pc) * fc_err_child)**2 +
                        (2 * np.pi * child_fc * np.sin(2 * np.pi * child_pc) *
                         pc_err_child)**2)
                    err_child_Im = np.sqrt(
                        (np.sin(2 * np.pi * child_pc) * fc_err_child)**2 +
                        (2 * np.pi * child_fc * np.cos(2 * np.pi * child_pc) *
                         pc_err_child)**2)
                    av_weighting_norm_factor_Re = av_weighting_norm_factor_Re + 1 / (
                        err_child_Re)**2
                    av_weighting_norm_factor_Im = av_weighting_norm_factor_Im + 1 / (
                        err_child_Im)**2
                    av_Re = av_Re + child_fc * np.cos(
                        2 * np.pi * child_pc) / (err_child_Re)**2
                    av_Im = av_Im + child_fc * np.sin(
                        2 * np.pi * child_pc) / (err_child_Im)**2
                ...
                av_err_Im_StdDev = 0
                for child_index in range(current_gp.childCount(
                )):  # (as av_Re and av_Im are required, we need a second loop!)
                    try:
                        child_pc = float(
                            current_gp.child(child_index).text(
                                self.Argand_col['Pc']))
                    ...
                    av_err_Re_StdDev = av_err_Re_StdDev + (
                        child_fc * np.cos(2 * np.pi * child_pc) - av_Re)**2
                    av_err_Im_StdDev = av_err_Im_StdDev + (
                        child_fc * np.sin(2 * np.pi * child_pc) - av_Im)**2
                av_err_Re_StdDev = np.sqrt(av_err_Re_StdDev /
                                           (current_gp.childCount() - 1))
                av_err_Im_StdDev = np.sqrt(av_err_Im_StdDev /
                                           (current_gp.childCount() - 1))
                ...
                av_err_pc_ErrorPropagation = np.sqrt(
                    (av_Im * av_err_Re / (2 * np.pi * av_fc**2))**2 +
                    (av_Re * av_err_Im / (2 * np.pi * av_fc**2))**2)
                av_err_pc_StdDev = np.sqrt((av_Im * av_err_Re_StdDev /
                                            (2 * np.pi * av_fc**2))**2 +
                                           (av_Re * av_err_Im_StdDev /
                                            (2 * np.pi * av_fc**2))**2)
                av_err_fc_ErrorPropagation = np.sqrt(
                    (av_Re * av_err_Re / av_fc)**2 +
                    (av_Im * av_err_Im / av_fc)**2)
                av_err_fc_StdDev = np.sqrt(
                    (av_Re * av_err_Re_StdDev / av_fc)**2 +
                    (av_Im * av_err_Im_StdDev / av_fc)**2)
                ...
                current_gp.setText(
                    self.Argand_col['Pc_err'],
                    str(max(av_err_pc_ErrorPropagation, av_err_pc_StdDev)))
                current_gp.setText(
                    self.Argand_col['Fc_err'],
                    str(max(av_err_fc_ErrorPropagation, av_err_fc_StdDev)))

        if refresh:
            self.Argand_replotGroupAverage(
                current_gp)  # replot the group average vector
        self.Argand_QTreeWidgetSignalsSwitch(on=True)
    ...
    def Argand_QTreeWidgetSignalsSwitch(self, on=-1):
        ...
        # connecting/disconnecting singals sensitive to changings in the TreeWidget
        if on:
            self.ui.treeWidget_Argand_List.itemDoubleClicked.connect(
                self.Argand_editColumn)
            self.ui.treeWidget_Argand_List.itemClicked.connect(
                self.Argand_editColumn)
            self.ui.treeWidget_Argand_List.itemSelectionChanged.connect(
                self.Argand_selectionChanged)
            self.ui.treeWidget_Argand_List.itemChanged.connect(
                lambda item, col: self.Argand_updateItems(
                    item_list=[item], col=col, recalcGP=True))
        else:
    ...
    def Argand_isDataPoint(self, item):
        return (item.parent() is not self.ui.treeWidget_Argand_List and
                not self.Argand_isGroup(item))
    ...
    def Agand_currentColor(self):
        ...
        return ci.background(
            self.Argand_col['Color']).color().getRgb()[0:3]  #This return also alpha

    def Argand_selectionChanged(self):
        items = self.ui.treeWidget_Argand_List.selectedItems()
        self.argand.markSelected(list(map(id, items)))
    ...
    def Argand_editColumn(self, item_clicked, col):
        ...
        if not self.Argand_isGroup(
                item_clicked
        ):  # make sure not to change group item flags (they are always non editable since they are calculated)
            signalsON = self.Argand_QTreeWidgetSignalsSwitch(
                on=False)  #changing flags shall not emit signals
            # Use the standard edit mode of QTreeWidget for other columns that should be editable
            if col in self.editableColumns:
                item_clicked.setFlags(
                    QtCore.Qt.ItemFlags(QtCore.Qt.ItemIsSelectable
        ...
        if self.TreeWidgetMouseButtonReleasedEvent == QtCore.Qt.RightButton or\
           self.TreeWidgetMouseButtonReleasedEvent == QtCore.QEvent.MouseButtonDblClick:
            ...
            if col == self.Argand_col['Name']:
                ...
                dialog.ui.lineEdit_newName.setText(item_list[0].text(
                    self.Argand_col['Name']))
                if dialog.exec_():
            ...
            # Ask the user to choose a color if the porper column is clicked.
                ...
                if color.isValid(
                ):  # color is invalid if the user aborts choosing a color
                    item_list = self.ui.treeWidget_Argand_List.selectedItems()
                    for item in item_list:
    ...
    def Argand_GpBySlice(self):
        ...
        if dialog.exec_():  # if OK is clicked
            ...
            for item in item_set_full:
                ...
                slices_set.add(
                    int(float(item.text(self.Argand_col['Slice nb']))))
            ...
            for slice_nb in slices_set:
                ...
                gp_dict = {}
                gp_dict.update({
                    'Name': '{:02d}_'.format(slice_nb) + suffix,
                    'Color': '({:d},{:d},{:d})'.format(color, 0, 255 - color)
                })
                new_gp = self.Argand_AddGroup(gp_dict, refresh=False)
                for item in item_set_full:
                    if int(float(item.text(
                            self.Argand_col['Slice nb']))) == slice_nb:
    ...
    def Argand_splitVector(self):
        ...
        if self.ui.checkBox_splitVector.isChecked() and current_item != None:
            ...
            if fc_split < fc_sum:
                print(
                    "WARNING: Invalid initial value for split vector. fc_split has to be greater than fc_sum"
                )
                print(
                    "         This can happen if the vector you are trying to split has a fraction greater than 1"
                )
                print(
                    "         If that is the case: Why do you want to split it anyway?"
                )
                print(
                    "         However, resetted initial split fraction from 1.0 to yourFc+0.1"
                )
                fc_split = fc_sum + 0.1

            delta_p = np.arccos(fc_sum / fc_split) / (
                2 * np.pi)  # magical equation from markus
            pcA = pc_sum - delta_p
            pcB = pc_sum + delta_p
            fcA = fcB = fc_split
            self.argand.addSplitVector(pcA, fcA, pc_sum, fc_sum, pcB, fcB, nA)
            ...
            self.proxy = pg.SignalProxy(
                self.argand.splitVectorList[-1].node_mouse_moved,
                rateLimit=60,
                slot=self.Argand_update_splitVector_position_proxy)
            self.argand.splitVectorList[-1].SigDragFinished.connect(
                lambda pos: self.Argand_splitMoveFinished(pos))
    ...
    def Argand_splitVector_symmetricRepositioning_update(self):
        if self.ui.checkBox_symmetricSplit.isChecked():
            ...
            self.Argand_splitVector_symmetricRepositioning(
                self.argand.splitVectorList[-1].getPosArray()[1])
        else:
            pass
    ...
    def Argand_splitVector_symmetricRepositioning(self, pos_cartesian_A):
        if self.ui.checkBox_symmetricSplit.isChecked():
            ...
            xA_new, yA_new = np.linalg.solve(
                [[-m_orth, 1], [-m_sum, 1]],
                [b_orth, b_2])  # [coeffs],[ordinates]
    ...
    def Argand_update_SplitVector_plot(self):
        xA, yA = self.argand.convertPcFc_to_cartesian(
            self.ui.doubleSpinBox_Argand_pcA.value(),
            self.ui.doubleSpinBox_Argand_fcA.value())
    ...
    def Argand_update_splitVector_position(self, pos_cartesian_A):
        ...
        self.argand.moveNode_i(self.argand.splitVectorList[0], 2, pc_sum,
                               fc_sum)  # updates when changing current item
        self.argand.moveNode_i(self.argand.splitVectorList[0], 3, pcB, fcB)
        self.argand.moveNode_i(self.argand.splitVectorList[0], 4, pcA, fcA *
                               nA)  # adding two vectors with weighted fraction
        self.argand.moveNode_i(self.argand.splitVectorList[0], 5, pcB,
                               fcB * (1 - nA))
        self.ui.doubleSpinBox_Argand_pcA.setValue(pcA)
        self.ui.doubleSpinBox_Argand_fcA.setValue(fcA)
        self.ui.doubleSpinBox_Argand_pcB.setValue(pcB)
    ...
    def Argand_refresh_tree_and_plot(self):
        ...
        self.Argand_groupAverage(
            refresh=False)  # do not refresh because it is followed by replot
        ...
        if '' != file_path:
            try:
                ...
                with open(file_path, 'w') as csvfile:
                    ...
                    fieldnames = ['Type'] + ['checkState'
                                            ] + self.Argand_ColList + ['Path']
                    w = csv.DictWriter(
                        csvfile,
                        delimiter=';',
                        quoting=csv.QUOTE_NONNUMERIC,
                    ...
                    currentDict = {}
                    for i_gp in range(
                            root.childCount()):  # Iterate on the groups
                        gp = root.child(i_gp)
                        ...
                        for j_child in range(gp.childCount(
                        )):  # Iterate in all points in a group
                            point = gp.child(j_child)
                            # clear the dict, fill it with defaults, overwirte defaults with values present in the QTreeWidget
                            currentDict.clear()
                            currentDict.update(
                                self.Argand_default_data_dictionary)
    ...
    def Argand_Load(self):
        ...
        progressBarLOADING = QProgressBar()
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Open File", self.ui.LineEdit_CurrentWorkingDirectory.text(),
            "Torricelli summary file(*.csv);; All (*)")
        if '' != file_path:
            try:
                total_lines = len(open(file_path).readlines()
                                 )  # it really just takes <1s for ~10^6 lines
                with open(file_path, 'r') as csvfile:
                    version = ''.join([
                        c for c in csvfile.readline() if c.isdigit() or c == '.'
                    ])
                    # Ignore the first line (contains Torricelli version)
                    csvOutdated = Version(version) < Version(
                        '3.3.273'
                    )  # is True if the csv is from an old Torricelli Version
                    if csvOutdated:
                        self.ui.statusbar.showMessage(
                            'The csv file you are loading is outdated! (From Torricelli v3.3.272 or earlier',
                            10000)
                        QMessageBox.warning(self, "Warning", '<b>The csv file you want to load is outdated!</b><br>'\
                    ...
                    for i, row in enumerate(reader):
                        ...
                        progressBarLOADING.setValue(int((i + 3) / total_lines * 100))
                        ...
                        if 'Group' == row['Type']:
                            ...
                            if csvOutdated:
                                dic_group.update({
                                    'Name':
                                        dic_group.pop('Group Name', 'NewGroup')
                                })
                            ...
                            if csvOutdated:
                                dic_point.update(
                                    {'Name': dic_point.get('Path', 'NewPoint')})
    ...
    def Argand_sortOnOff(self):
        if self.ui.checkBox_SortOnOff.isChecked():
            ...
            self.ui.treeWidget_Argand_List.sortItems(self.Argand_col['Name'],
    ...
    def Argand_saveFitResult_and_plot(self):
        ...
        self.Argand_AddDataset(
            gp_liveAn, self.get_dict_with_all_values(), refresh=True)
        self.Argand_Save(
            str(self.ui.LineEdit_CurrentWorkingDirectory.text()) + os.sep +
            self.argand_SaveSubFolder + 'autosave_' + self.timestamp() +
            '_newFitResultAdded.csv')
    ...
    def get_dict_with_all_values(self):
        dic = self.Argand_default_data_dictionary.copy()
        name_str = str(self.ui.LineEdit_CurrentWorkingDirectory.text()).split(
            os.sep)[-1]
    ...
    def get_final_Psi(self):
        if not self.ui.radioButton_EYinit_man_SR.isChecked():
            if self.ui.checkBox_eyfit_fitgamma.isChecked():
                return self.ui.doubleSpinBox_EYinit_Psi_fitResult.value()
            else:
                return self.ui.doubleSpinBox_EYinit_Psi.value()
        else:
            return '-'
    ...
    def get_final_Q0(self):
        if not self.ui.radioButton_EYinit_man_SR.isChecked():
            if self.ui.checkBox_eyfit_fitgamma.isChecked():
                return self.ui.doubleSpinBox_EYinit_Q0_fitResult.value()
            else:
                return self.ui.doubleSpinBox_EYinit_Q0.value()
        else:
            return '-'
    ...
    def get_final_QH(self):
        if not self.ui.radioButton_EYinit_man_SR.isChecked():
            if self.ui.checkBox_eyfit_fitgamma.isChecked():
                return self.ui.doubleSpinBox_EYinit_QH_fitResult.value()
            else:
                return self.ui.doubleSpinBox_EYinit_QH.value()
        else:
            return '-'

    def get_final_Pc(self):
        if self.ui.doubleSpinBox_EYFit_fc.value() > 1e-4:
            if self.ui.doubleSpinBox_EYFit_pc.isEnabled():
                return self.ui.doubleSpinBox_EYFit_pc.value()
            else:
                return self.ui.doubleSpinBox_pc.value()
        else:
            return '-'

    def get_final_Si(
            self):  # Function necessary to self.Argand_saveFitResult_and_plot()
        if self.ui.doubleSpinBox_EYFit_fc.value() > 1e-4:
            if self.ui.doubleSpinBox_EYinit_abs_si_fitResult.isEnabled():
                return self.ui.doubleSpinBox_EYinit_abs_si_fitResult.value()
            else:
                return self.ui.doubleSpinBox_EYinit_abs_si.value()
        else:
            return '-'
    ...
    def get_final_Psi(
        ...
        if self.ui.doubleSpinBox_EYFit_fc.value() > 1e-4:
            if not self.ui.radioButton_EYinit_man_SR.isChecked():
                if self.ui.doubleSpinBox_EYinit_Psi_fitResult.isEnabled():
                    return self.ui.doubleSpinBox_EYinit_Psi_fitResult.value()
                else:
                    return self.ui.doubleSpinBox_EYinit_Psi.value()
            else:
    ...
    def get_final_Q0(
            self):  # Function necessary to self.Argand_saveFitResult_and_plot()
        if self.ui.doubleSpinBox_EYFit_fc.value() > 1e-4:
            if self.ui.doubleSpinBox_EYinit_Q0_fitResult.isEnabled():
                return self.ui.doubleSpinBox_EYinit_Q0_fitResult.value()
            else:
                return self.ui.doubleSpinBox_EYinit_Q0.value()
        else:
    ...
    def get_final_QH(
            self):  # Function necessary to self.Argand_saveFitResult_and_plot()
        if self.ui.doubleSpinBox_EYFit_fc.value() > 1e-4:
            if self.ui.doubleSpinBox_EYinit_QH_fitResult.isEnabled():
                return self.ui.doubleSpinBox_EYinit_QH_fitResult.value()
            else:
                return self.ui.doubleSpinBox_EYinit_QH.value()
        else:
    ...
    def get_final_gamma(
        ...
        if self.ui.radioButton_EYinit_man_SR.isChecked():
            ...
            if self.ui.checkBox_eyfit_fitgamma.isChecked():
                return self.ui.doubleSpinBox_EYFit_gamma.value()
            else:
                return self.get_init_value_gamma()

    def get_final_gamma_err(
            self):  # Function necessary to self.Argand_saveFitResult_and_plot()
        if not self.ui.radioButton_EYinit_man_SR.isChecked(
        ) and self.ui.checkBox_eyfit_fitgamma.isChecked():
            return self.ui.doubleSpinBox_stddev_gamma.value()
        else:
            return '-'
        ...
        for vec in self.argand.splitVectorList:
            if checked:
                splitvec_label = [
                    '', '<big>F<sub>A</sub></big>', '',
                    '<big>F<sub>B</sub></big>',
                    'n<sub>A</sub>&middot;F<sub>A</sub>',
                    'n<sub>B</sub>&middot;F<sub>B</sub>'
                ]
            else:
                splitvec_label = ['', '', '', '', '', '']
            ...
            for i in range(child_count):
                ...
                if gp.checkState(self.Argand_col['Name']) == QtCore.Qt.Checked:
                    ...
                    if checked:
                        originvec_label = [
                            '', '<big>' +
                            str(gp.text(self.Argand_col['Name'])) + '</big>'
                        ]
                    else:
                        originvec_label = ['', '']
    ...
    def Argand_updateFcPcGrid(self, *args):
        self.argand.changeAxisAndGrid(
            polarGridNr=self.ui.spinBox_Fc_grid_lines.value(),
            radialGridNr=self.ui.spinBox_Pc_grid_lines.value())
        #self.Argand_replotDiagram()

    def ArgandList_editable(self):
        if self.ui.checkBox_ArgandList_editable.isChecked():
            self.editableColumns = list(
                map(self.Argand_col.get, self.editableColList))
        else:
            self.editableColumns = list(
                map(self.Argand_col.get, self.noteditableColList))
    ...
    def set_all_ToolTips(self):  #self.ui..setToolTip('')
        ...
        self.ui.doubleSpinBox_sample_P_Refl.setToolTip(
            'Polarization factor for the x-ray')
        self.ui.doubleSpinBox_sample_miscut.setToolTip(
            'Miscut.\nShould be close to 0' + deg + find_Geometry)
        self.ui.doubleSpinBox_sample_deviation_NI.setToolTip(
            'Deviation from normal incidence.\nPositive values mean that the sample normal points towards the analyzer.\nShould be close to 0'
            + deg + find_Geometry)
        self.ui.doubleSpinBox_theta.setToolTip(
            'Angle between the incoming beam and the atomic plane\n' + theta +
            ' = 90-' + xi + find_Geometry)
        self.ui.doubleSpinBox_b_cr.setToolTip('Asymmetry parameter')
        self.ui.doubleSpinBox_theoReflRange.setToolTip(
            'Here you can set the photon energy range for calculating the theoretical reflectivity.\nThis range must be wider than your experimental reflectivity otherwise the\nfitting of the reflectivity will fail because interpolation of your experimental\ndata will exceed the range of available datapoints!\n\n\tRecommended value: 8 eV'
        )
        self.ui.radioButton_sigma_pol_light.setToolTip(
            'Electric field perpendicular to the plane of incidence')
        self.ui.radioButton_pi_pol_light.setToolTip(
        ...
        self.ui.spinBox_SelectedSlice.setToolTip(
            'Slice number. The 0th slice corresponds to the smallest phi value.'
            + find_Geometry)
        ...
        self.ui.checkBox_AngularModeToggle.setToolTip(
            'Use this if you concatenated several XSW data sets in a\nsingle casaXPS file (e.g. for fitting many slices at once)'
            + find_Geometry)
        ...
        self.ui.checkBox_ignore_MonteCarlo.setToolTip(
            'Allows the user to ignore the statistical errors.\n(Note however that the statistical errors columns must still be present in the yield file, even they are only filled with zeros)\nTo be used ONLY for fast analysis during a beam-time!'
        )
        self.ui.checkBox_ignore_hvCheck.setToolTip(
            'Check this if the photon energies are wrongly defined in the yield file.'
        )
        self.ui.label_angles_file_info.setToolTip(
            'Information stored in the 2nd and 3rd header line of the angle file'
        )
        ...
        ### Fit Yield
        self.ui.checkBox_display_manual_EY.setToolTip(
            'Displays properly only after having fitted once')
        ...
        self.ui.comboBox_NonDipolar_Shells.setToolTip(
            'Non-dipolar parameter theory valid for s-subshells only!')
        self.ui.label_20.setToolTip(
            'Non-dipolar parameter theory valid for s-subshells only!')
        ...
        ### Argand diagram
        self.ui.checkBox_splitVector.setToolTip(
            'Decompose the currenlty selected point into two vector A and B.')
        self.ui.checkBox_symmetricSplit.setToolTip(
            'Force the fraction of A and B to be the same')
    ...
    def PrepareGUI_InitVariables(self):
        self.setWindowTitle(
            'Torricelli __ The friendly X-Ray Standing Wave data analysis software __ ver'
            + __version__)
        self.setWindowIcon(QIcon('imports/Torricelli_icon.png'))
        ...
        self.ui.label_fit_ey_chi.setPixmap(
            QPixmap(Torricelli_program_folder_path + os.sep + 'imports' +
                    os.sep + 'chi_red2.png'))
        #self.ui.label_refl_chi.setScaledContents(False)
        self.ui.label_refl_chi.setPixmap(
            QPixmap(Torricelli_program_folder_path + os.sep + 'imports' +
                    os.sep + 'chi_2_Pearson.png'))
        self.ui.label_initialChiSquared.setPixmap(
            QPixmap(Torricelli_program_folder_path + os.sep + 'imports' +
                    os.sep + 'chi_red2.png'))
        ...
        self.E_b = 0
        self.nb_slices = 0
        self.Components_List_Used_in_EYfit = np.array(
            [0])  # default value (compelled by the np.fromstring function)
        self.R_squared_ey = 0
        ...
        self.Argand_col = dict(
            list(
                zip(self.Argand_ColList,
                    list(range(len(self.Argand_ColList))))))
        # Set the header ot the TreeWidget according to Argand_ColList
        self.ui.treeWidget_Argand_List.setHeaderLabels(
            list(self.Argand_ColList))
        self.ui.treeWidget_Argand_List.setEditTriggers(
            QAbstractItemView.DoubleClicked
            | QAbstractItemView.EditKeyPressed
        )  # sets the events to enter into the edit mode
        self.editableColList = [
            'Component', 'Core level', 'Phi', 'Element', 'Subshell',
            'Components', 'Slice nb', 'Phi', 'Pc', 'Fc', 'Pc_err', 'Fc_err',
            'Gamma', 'Gamma_err', 'Q_0', 'Q_H', 'Delta', 'P el', 'Sr', 'Sr_err',
            '|Si|', 'Psi', 'Zeta', 'b sample', 'b DCM', 'Xi', 'Pol.', 'P Refl',
            'Substrate', '(hkl)', 'DW', 'Temp.', 'delta hnu', 'Sigma',
            'R2 Refl', 'X2 Yield', 'Monte Carlo analysis', 'Note'
        ]
        ...
        self.Argand_default_data_dictionary = dict.fromkeys(
            self.Argand_ColList, '')
        self.Argand_default_group_dictionary = dict.fromkeys(
            self.Argand_ColList, '')
        ...
        self.ui.label_ArgandVersion.setText('pyArgand v' +
                                            str(self.argand.__version__))
    ...
    def prepare_the_plot_panels(self):
        ...
        self.Pyqt_View_raw_refl = pg.PlotWidget(
            title='I0-Normalised reflectivity')
        self.Pyqt_View_raw_refl.setLabel(
            'left', "Reflectivity", units=' arb. units')
        self.Pyqt_View_raw_refl.setLabel('bottom', "Photon energy", units='eV')
        self.Pyqt_View_raw_refl.showGrid(
            x=True, y=False, alpha=grid_transparency)
        self.Pyqt_View_raw_refl.showAxis(
            'top',
            show=True)  #('top', showValues=False) # Where to write this ?!!
        self.ui.horizontalLayout_raw_refl.addWidget(self.Pyqt_View_raw_refl)
        self.Pyqt_View_raw_refl.enableAutoRange()

        self.Pyqt_View_raw_ey = pg.PlotWidget(
            title='Summed electron yield of given components (I0-normalised)')
        self.Pyqt_View_raw_ey.setLabel(
            'left', "Electron yield", units=' arb. units')
        ...
        self.Pyqt_View_idealRefl = pg.PlotWidget(
            title='Ideal Reflectivities and Phase')
        self.Pyqt_View_idealRefl.setLabel('left', "Reflectivity or Phase/Pi")
        self.Pyqt_View_idealRefl.setLabel(
            'bottom',
            "Photon energy relative to theoretical Bragg energy",
            units='eV')
        self.Pyqt_View_idealRefl.showGrid(
            x=True, y=True, alpha=grid_transparency)
        ...
        self.Pyqt_View_refl_fit.setLabel(
            'left', "Reflectivity", units=' arb. units')
        self.Pyqt_View_refl_fit.setLabel(
            'bottom',
            "Photon energy relative to theoretical Bragg energy",
            units='eV')
        self.Pyqt_View_refl_fit.showGrid(
            x=True, y=True, alpha=grid_transparency)
        ...
        self.Pyqt_View_ey_fit.setLabel(
            'left', 'Normalised Photoelectron yield', units=' arb. units')
        self.Pyqt_View_ey_fit.setLabel(
            'bottom',
            "Photon energy relative to theoretical Bragg energy",
            units='eV')
    ...
    def closeEvent(self, event):
        ...
        user_file.write(
            str(self.ui.doubleSpinBox_sample_deviation_NI.value()) + '\n')
        user_file.write(str(self.ui.doubleSpinBox_sample_miscut.value()) + '\n')
        user_file.write(
            str(self.ui.radioButton_Sample_Elemental.isChecked()) +
            '\n')  # Elemental or Compound
        user_file.write(
            str(self.ui.comboBox_Sample_Elemental_Element.currentIndex()) +
            '\n')
        user_file.write(
            str(self.ui.comboBox_Sample_Compound_Element.currentIndex()) + '\n')
        ...
        user_file.write(
            str(self.ui.spinBox_NonDipolar_Element_Z.value()) +
            '\n')  # Z element
        ...
        user_file.write(str(self.ui.doubleSpinBox_ndp_phi.value()) +
                        '\n')  # phi
        user_file.write(
            str(self.ui.doubleSpinBox_EYinit_man_gamma.value()) +
            '\n')  # manual gamma
        user_file.write(
            str(self.ui.doubleSpinBox_EYinit_man_SR.value()) +
            '\n')  # manual sr
        ...
        if root.childCount() > 0:
            self.Argand_Save(
                ...
                '_lastSession.csv')
        event.accept(
        )  # in this particular case, the closeEvent(), one should keep the event.accept(), as advised here:
    ...
    def read_in_user_settings(self):
        try:
            with open(
                    Torricelli_program_folder_path + os.sep + 'imports' +
                    os.sep + 'user_settings', 'r') as f:
                self.ui.LineEdit_CurrentWorkingDirectory.setText(
                    f.readline().rstrip('\n'))
                self.choose_dataFolder(
                    self.ui.LineEdit_CurrentWorkingDirectory.text())

                # Tab theoretical reflectivity and phase
                self.ui.doubleSpinBox_theoReflRange.setValue(
                    float(f.readline().rstrip('\n')))
                self.ui.doubleSpinBox_sample_deviation_NI.setValue(
                    float(f.readline().rstrip('\n')))
                self.ui.doubleSpinBox_sample_miscut.setValue(
                    float(f.readline().rstrip('\n')))
                if f.readline().rstrip('\n') in ['True']:
                    self.ui.radioButton_Sample_Elemental.setChecked(True)
                else:
                    self.ui.radioButton_Sample_Compound.setChecked(True)
                self.ui.comboBox_Sample_Elemental_Element.setCurrentIndex(
                    int(f.readline().rstrip('\n')))
                self.ui.comboBox_Sample_Compound_Element.setCurrentIndex(
                    int(f.readline().rstrip('\n')))
                self.ui.spinBox_Sample_Elemental_h.setValue(
                    int(f.readline().rstrip('\n')))
                self.ui.spinBox_Sample_Elemental_k.setValue(
                    int(f.readline().rstrip('\n')))
                self.ui.spinBox_Sample_Elemental_l.setValue(
                    int(f.readline().rstrip('\n')))
                self.ui.spinBox_Sample_Compound_h.setValue(
                    int(f.readline().rstrip('\n')))
                self.ui.spinBox_Sample_Compound_k.setValue(
                    int(f.readline().rstrip('\n')))
                self.ui.spinBox_Sample_Compound_l.setValue(
                    int(f.readline().rstrip('\n')))
                ...
                self.ui.spinBox_NonDipolar_Element_Z.setValue(
                    int(f.readline().rstrip('\n')))
                self.ui.doubleSpinBox_ndp_delta_p.setValue(
                    float(f.readline().rstrip('\n')))
                self.ui.doubleSpinBox_ndp_delta_d.setValue(
                    float(f.readline().rstrip('\n')))
                self.ui.doubleSpinBox_ndp_phi.setValue(
                    float(f.readline().rstrip('\n')))
                self.ui.doubleSpinBox_EYinit_man_gamma.setValue(
            ...
            self.ui.statusbar.showMessage(
                "Default folder and file number used.", 5000)
    ...
    def angularModeOption(self):
        ...
        if self.ui.checkBox_AngularModeToggle.isChecked():
            self.ui.statusbar.showMessage(
                "NOTE: You are using the angular-resolved functionality: usual photon energy checks on the data are not performed!"
            )

            self.Transform_R_and_EY_userFile_to_TorricelliFriendly(
                calledFromAngularCheckBox=True)
    ...
    def Connect_QtWidgets_and_Functions(self):
        root = self.ui.treeWidget_Argand_List.invisibleRootItem()
        self.ui.button_CurrentWorkingDirectory.clicked.connect(
            self.choose_dataFolder)
        self.ui.signal_name.textEdited.connect(self.update_component_list)
        ## Section Structure Factors
        self.ui.comboBox_DCM_Element.currentIndexChanged.connect(
            self.update_structure_DCM)
        self.ui.doubleSpinBox_DCM_Temperature.valueChanged.connect(
            self.update_structure_DCM)
        self.ui.comboBox_Sample_Elemental_Element.currentIndexChanged.connect(
            self.update_structure_Sample_Elemental)
        self.ui.doubleSpinBox_Sample_Elemental_Temperature.valueChanged.connect(
            self.update_structure_Sample_Elemental)
        self.ui.comboBox_Sample_Compound_Element.currentIndexChanged.connect(
            self.update_structure_Sample_Compound)
        self.ui.doubleSpinBox_Sample_Compound_Temperature.valueChanged.connect(
            self.update_structure_Sample_Compound)
        self.ui.comboBox_DCM_Element.setCurrentIndex(2)  #Si
        self.update_structure_Sample_Compound(
            0
        )  # As 0 is default, the currentIndexChanged is not send, and therefore no Structure is displayed
        self.ui.pushButton_calculateAllStructureFactors.clicked.connect(
            self.calculate_structure_factors)
        self.ui.pushButton_editDCM.clicked.connect(
            self.allow_DCM_parameter_modification)
        self.ui.radioButton_Sample_Elemental.clicked.connect(
            self.Sample_ElementalCompound_choice_changed)
        self.ui.radioButton_Sample_Compound.clicked.connect(
            self.Sample_ElementalCompound_choice_changed)
        self.ui.spinBox_DCM_h.valueChanged.connect(
            self.refresh_forth_Miller_Bravais_indice)
        self.ui.spinBox_DCM_k.valueChanged.connect(
            self.refresh_forth_Miller_Bravais_indice)
        self.ui.spinBox_Sample_Elemental_h.valueChanged.connect(
            self.refresh_forth_Miller_Bravais_indice)
        self.ui.spinBox_Sample_Elemental_k.valueChanged.connect(
            self.refresh_forth_Miller_Bravais_indice)
        self.ui.spinBox_Sample_Compound_h.valueChanged.connect(
            self.refresh_forth_Miller_Bravais_indice)
        self.ui.spinBox_Sample_Compound_k.valueChanged.connect(
            self.refresh_forth_Miller_Bravais_indice)
        self.ui.doubleSpinBox_sample_deviation_NI.valueChanged.connect(
            self.refresh_sample_b_and_theta_value)
        self.ui.doubleSpinBox_sample_miscut.valueChanged.connect(
            self.refresh_sample_b_and_theta_value)
        self.ui.doubleSpinBox_sample_deviation_NI.valueChanged.connect(
            self.refresh_sample_P_value)
        self.ui.radioButton_pi_pol_light.clicked.connect(
            self.refresh_sample_P_value)
        self.ui.radioButton_sigma_pol_light.clicked.connect(
            self.refresh_sample_P_value)
        ...
        self.ui.doubleSpinBox_Sample_Compound_Temperature.valueChanged.connect(
            self.clear_structFact_display)
        self.ui.comboBox_DCM_DWMethod.currentIndexChanged.connect(
            self.clear_structFact_display)
        self.ui.comboBox_Sample_Elemental_DWMethod.currentIndexChanged.connect(
            self.clear_structFact_display)
        self.ui.comboBox_Sample_Compound_DWMethod.currentIndexChanged.connect(
            self.clear_structFact_display)
        self.ui.spinBox_DCM_h.valueChanged.connect(
            self.clear_structFact_display)
        self.ui.spinBox_DCM_k.valueChanged.connect(
            self.clear_structFact_display)
        self.ui.spinBox_DCM_l.valueChanged.connect(
            self.clear_structFact_display)
        self.ui.spinBox_Sample_Elemental_h.valueChanged.connect(
            self.clear_structFact_display)
        self.ui.spinBox_Sample_Elemental_k.valueChanged.connect(
            self.clear_structFact_display)
        self.ui.spinBox_Sample_Elemental_l.valueChanged.connect(
            self.clear_structFact_display)
        self.ui.spinBox_Sample_Compound_h.valueChanged.connect(
            self.clear_structFact_display)
        self.ui.spinBox_Sample_Compound_k.valueChanged.connect(
            self.clear_structFact_display)
        self.ui.spinBox_Sample_Compound_l.valueChanged.connect(
            self.clear_structFact_display)
        ## Section Import Files
        ...
        self.ui.button_import.clicked.connect(
            self.Transform_R_and_EY_userFile_to_TorricelliFriendly)
        self.ui.checkBox_AngularModeToggle.toggled.connect(
            self.angularModeOption)
        self.ui.spinBox_SelectedSlice.valueChanged.connect(
            self.Transform_R_and_EY_userFile_to_TorricelliFriendly)
        ## Section Fit Reflectivity
        ...
        self.ui.comboBox_NonDipolar_Element.currentIndexChanged.connect(
            self.nonDipolar_changeElement)
        self.ui.spinBox_NonDipolar_Element_Z.valueChanged.connect(
            self.nonDipolar_changeElementZ)
        self.ui.comboBox_NonDipolar_Shells.currentIndexChanged.connect(
            lambda val: self.display_Gamma_file(val))
        self.ui.pushButton_exportGammaFile.clicked.connect(self.saveGammaFile)
        ## Subection Fit EY
        self.ui.pushButton_reset_initial_parameters.clicked.connect(
            self.reset_ey_par)
        self.ui.button_fit_ey.clicked.connect(self.fit_ElYield)
        self.ui.pushButton_set_fitParam_for_manual.clicked.connect(
            self.set_fitEYparam_forManualUse)
        self.ui.horizontalSlider_manual_fc.sliderMoved.connect(
            lambda val, who="fc": self.EY_initSlider_valueChanged(who))
        self.ui.horizontalSlider_manual_fc.actionTriggered.connect(
            lambda val, who="fc": self.EY_initSlider_valueChanged(who))
        self.ui.doubleSpinBox_fc.valueChanged.connect(
            lambda val, who="fc": self.EY_initSpin_valueChanged(who))
        self.ui.horizontalSlider_manual_pc.sliderMoved.connect(
            lambda val, who="pc": self.EY_initSlider_valueChanged(who))
        self.ui.horizontalSlider_manual_pc.actionTriggered.connect(
            lambda val, who="pc": self.EY_initSlider_valueChanged(who))
        self.ui.doubleSpinBox_pc.valueChanged.connect(
            lambda val, who="pc": self.EY_initSpin_valueChanged(who))
        self.ui.checkBox_display_manual_EY.clicked.connect(self.update_EY_plot)
        self.ui.checkBox_ignore_MonteCarlo.toggled.connect(
            self.update_chiSquared)
        self.ui.pushButton_saveToArgand.clicked.connect(
            self.Argand_saveFitResult_and_plot)
        self.ui.radioButton_EYinit_theo_gamma.toggled.connect(
            self.ndp_choose_work_with_SR_or_gamma)
        self.ui.radioButton_EYinit_man_gamma.toggled.connect(
            self.ndp_choose_work_with_SR_or_gamma)
        self.ui.radioButton_EYinit_man_SR.toggled.connect(
            self.ndp_choose_work_with_SR_or_gamma)
        self.ui.checkBox_eyfit_fitFc.clicked.connect(
            self.ndp_choose_work_with_SR_or_gamma)
        self.ui.doubleSpinBox_fc.valueChanged.connect(
            self.ndp_choose_work_with_SR_or_gamma)
        self.ui.checkBox_eyfit_fitPc.clicked.connect(
            self.ndp_choose_work_with_SR_or_gamma)
        self.ui.checkBox_eyfit_fitN.clicked.connect(
            self.ndp_choose_work_with_SR_or_gamma)
        self.ui.checkBox_eyfit_fitgamma.clicked.connect(
            self.ndp_choose_work_with_SR_or_gamma)
        self.ui.checkBox_eyfit_fitSr.clicked.connect(
            self.ndp_choose_work_with_SR_or_gamma)
        self.ui.doubleSpinBox_ndp_delta_p.valueChanged.connect(
            self.updatePhaseShiftDifference)
        self.ui.doubleSpinBox_ndp_delta_d.valueChanged.connect(
            self.updatePhaseShiftDifference)
        self.ui.doubleSpinBox_ndp_phi.valueChanged.connect(
            self.refresh_sample_P_value)
        self.ui.spinBox_NonDipolar_Element_Z.valueChanged.connect(
            self.setPhotoelectronKineticEnergy)
        self.ui.comboBox_NonDipolar_Shells.currentIndexChanged.connect(
            self.setPhotoelectronKineticEnergy)
        self.ui.doubleSpinBox_ndp_EBragg.valueChanged.connect(
            self.setPhotoelectronKineticEnergy)
        ...
        self.ui.doubleSpinBox_EYinit_man_gamma.valueChanged.connect(
            self.setYieldParameters)
        self.ui.doubleSpinBox_EYinit_theo_gamma.valueChanged.connect(
            self.setYieldParameters)
        self.ui.radioButton_EYinit_theo_gamma.clicked.connect(
            self.setYieldParameters)
        self.ui.radioButton_EYinit_man_gamma.clicked.connect(
            self.setYieldParameters)
        self.ui.doubleSpinBox_EYinit_Delta.valueChanged.connect(
            self.setYieldParameters)
        self.ui.doubleSpinBox_EYinit_Pe.valueChanged.connect(
            self.setYieldParameters)
        ...
        self.ui.doubleSpinBox_ndp_Ekin.valueChanged.connect(
            self.setYieldParameters)

        ## Section: Argand diagram
        self.vb = self.argand.plotItem.vb  # define ViewBox of the ArgandPlotWidget
        self.ui.treeWidget_Argand_List.invisibleRootItem().setFlags(
            QtCore.Qt.ItemFlags(QtCore.Qt.ItemIsEnabled))
        self.Argand_QTreeWidgetSignalsSwitch(on=True)
        ...
        self.ui.pushButton_Argand_Remove.clicked.connect(
            lambda: self.Argand_Remove(self.ui.treeWidget_Argand_List.
                                       selectedItems()))
        self.ui.pushButton_Argand_removeAll.clicked.connect(
            self.Argand_RemoveAll)
        self.ui.pushButton_Argand_AddDataset.clicked.connect(
            self.Argand_ValueLogFile)
        self.ui.pushButton_Argand_ManualValues.clicked.connect(
            self.Argand_ManualValues)
        ...
        self.ui.checkBox_splitVector.clicked.connect(
            self.Argand_update_Split_display)
        self.ui.checkBox_splitVector.clicked.connect(self.Argand_splitVector)
        self.ui.doubleSpinBox_Argand_nA.valueChanged.connect(
            self.Argand_update_SplitVector_plot)
        self.ui.doubleSpinBox_Argand_pcA.valueChanged.connect(
            self.Argand_update_SplitVector_plot)
        self.ui.doubleSpinBox_Argand_fcA.valueChanged.connect(
            self.Argand_update_SplitVector_plot)
        self.ui.doubleSpinBox_Argand_nA.editingFinished.connect(
            self.Argand_splitVector_symmetricRepositioning_update)
        self.ui.doubleSpinBox_Argand_pcA.editingFinished.connect(
            self.Argand_splitVector_symmetricRepositioning_update)
        self.ui.doubleSpinBox_Argand_fcA.editingFinished.connect(
            self.Argand_splitVector_symmetricRepositioning_update)
        self.ui.checkBox_displayLabels.toggled.connect(
            lambda checked: self.Argand_Labels(checked))
        self.ui.checkBox_symmetricSplit.toggled.connect(
            self.Argand_splitVector_symmetricRepositioning_update)
        self.ui.pushButton_GpBySlice.clicked.connect(self.Argand_GpBySlice)
        self.ui.checkBox_Argand_display_originVectors.clicked.connect(
            lambda checked: self.Argand_updateItems(
                item_list=list(map(root.child, list(range(root.childCount())))),
                recalcGP=False))
        self.ui.checkBox_Argand_display_group_errorBars.clicked.connect(
            lambda checked: self.Argand_updateItems(
                item_list=list(map(root.child, list(range(root.childCount())))),
                recalcGP=False))
        self.ui.checkBox_Argand_display_errorBars.clicked.connect(
            self.Argand_replotDiagram)
        self.ui.pushButton_refresh.clicked.connect(
            self.Argand_refresh_tree_and_plot)
        self.ui.spinBox_Fc_grid_lines.editingFinished.connect(
            self.Argand_updateFcPcGrid)
        self.ui.spinBox_Pc_grid_lines.editingFinished.connect(
            self.Argand_updateFcPcGrid)
        self.ui.checkBox_SortOnOff.clicked.connect(self.Argand_sortOnOff)
        self.ui.checkBox_ArgandList_editable.clicked.connect(
            self.ArgandList_editable)

        ## Section: Geometry, About, License:
        self.ui.label_geometry.setPixmap(
            QPixmap(Torricelli_program_folder_path + os.sep + 'imports' +
                    os.sep + 'Geometry.png'))
        ...
        self.proxy = pg.SignalProxy(
            self.argand.plotItem.scene().sigMouseMoved,
            rateLimit=60,
    ...
    def keyPressEvent(self, event):
        ...
        if event.type(
        ) == QtCore.QEvent.KeyPress and 16777239 == key:  # This number is not elegent, Corresponds to Ctrl+PageUp
            self.ui.tab_Main.setCurrentIndex(self.ui.tab_Main.currentIndex() +
                                             1)
        elif event.type(
        ) == QtCore.QEvent.KeyPress and 16777238 == key:  #Ctrl+PageDown
            self.ui.tab_Main.setCurrentIndex(self.ui.tab_Main.currentIndex() -
                                             1)
    ...
    def mouseMoved(self, evt):
        ...
        if self.argand.plotItem.sceneBoundingRect().contains(self.pos):
            ...
            self.ui.label_livePc.setText(
                "Pc = %0.4f" %
                (np.arctan2(self.mousePoint.y(), self.mousePoint.x()) /
                 (2 * np.pi)
                 if np.arctan2(self.mousePoint.y(), self.mousePoint.x()) /
                 (2 * np.pi) >= 0 else
                 (np.arctan2(self.mousePoint.y(), self.mousePoint.x()) +
                  2 * np.pi) / (2 * np.pi)))
            self.ui.label_liveFc.setText(
                "Fc = %0.4f" %
                np.sqrt(self.mousePoint.x()**2 + self.mousePoint.y()**2))
